				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		val->intval = gpio_get_value(gpio_power->axp_acin_det.gpio);
		break;
	default:
		ret = -EINVAL;
	}
...
static irqreturn_t axp_acin_gpio_isr(int irq, void *data)
{
	...
	schedule_delayed_work(&gpio_power->gpio_chg_state, 1);
...
static void axp2202_gpio_power_set_current_fsm(struct work_struct *work)
{
	...
	acin_det_gpio_value = gpio_get_value(gpio_power->axp_acin_det.gpio);
