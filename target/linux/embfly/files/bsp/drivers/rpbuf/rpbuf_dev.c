...
static int rpbuf_buf_dev_ioctl_get_perf_data(struct rpbuf_buf_dev *buf_dev,
					   void __user *argp)
{
	struct device *dev = &buf_dev->dev;
	struct rpbuf_perf_data_ioctl __user *perf_data_user = argp;
	struct rpbuf_perf_data_ioctl perf_data_ioctl;
	struct rpbuf_perf_data perf_data;
	uint32_t target_len = sizeof(rpbuf_perf_data_t);

	int ret = 0;

	if (copy_from_user(&perf_data_ioctl, perf_data_user, sizeof(perf_data_ioctl))) {
		dev_err(dev, "copy_from_user rpbuf_perf_data failed\n");
		ret = -EIO;
		goto err_out;
	}

	if (perf_data_ioctl.len != target_len) {
		dev_err(dev, "perf_data is not same, kernel: %u, user: %u\n", target_len, perf_data_ioctl.len);
		ret = -EINVAL;
		goto err_out;
	}

	ret = rpbuf_get_perf_data(buf_dev->buffer, &perf_data);
	if (ret)
		goto err_out;

	if (copy_to_user(perf_data_ioctl.data, &perf_data, sizeof(perf_data))) {
		dev_err(dev, "copy_to_user rpbuf_perf_data failed\n");
		ret = -EIO;
	}
				...
				void __user *argp)
{
	...
	case RPBUF_BUF_DEV_IOCTL_SET_SYNC_BUF:
		...
		break;
	case RPBUF_BUF_DEV_IOCTL_GET_PERF_DATA:
		ret = rpbuf_buf_dev_ioctl_get_perf_data(buf_dev, argp);
		break;
	default:
