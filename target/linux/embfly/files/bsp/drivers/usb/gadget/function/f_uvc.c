...
#include <uapi/linux/usb/sunxi_uapi_uvc.h>
...
module_param(bulk_streaming_ep, bool, 0644);
...
static char uvc_default_control_interface_strings[] = "UVC Camera";
static char uvc_default_streaming_interface_strings[] = "Video Streaming";
static struct usb_string uvc_en_us_strings[] = {
	[UVC_STRING_CONTROL_IDX].s = uvc_default_control_interface_strings,
	[UVC_STRING_STREAMING_IDX].s = uvc_default_streaming_interface_strings,
	{  }
};
...
uvc_function_set_alt(struct usb_function *f, unsigned interface, unsigned alt)
{
	...
	pr_err("%s:%d interface %d set alt %d\n", __FUNCTION__, __LINE__, interface, alt);
...
uvc_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	if (gadget_is_superspeed(c->cdev->gadget)) {
		...
		if (!bulk_streaming_ep) {
			...
			/* FIXME:
			 * In bulk mode, do usb_ep_enable before usb_ep_alloc_request,
			 * and desc->wMaxPacketSize limited 64 by usb_ep_autoconfig,
			 * it make bulk ep size 64 unexpectedly, so fix it to 512
			 */
			uvc_hs_bulk_streaming_ep.wMaxPacketSize =
				min(opts->streaming_maxpacket, 512U);
		}
	...
	if (strlen(opts->control_interface_strings))
		uvc_en_us_strings[UVC_STRING_CONTROL_IDX].s =
					opts->control_interface_strings;
	if (strlen(opts->streaming_interface_strings))
		uvc_en_us_strings[UVC_STRING_STREAMING_IDX].s =
					opts->streaming_interface_strings;
	uvcg_info(f, "uvc control interface strings: %s\n",
			uvc_en_us_strings[UVC_STRING_CONTROL_IDX].s);
	uvcg_info(f, "uvc streaming interface strings: %s\n",
			uvc_en_us_strings[UVC_STRING_STREAMING_IDX].s);
