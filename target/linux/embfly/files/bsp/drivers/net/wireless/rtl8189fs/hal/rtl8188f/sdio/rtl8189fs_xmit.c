...
query_free_page:
	...
	if (_FALSE == rtw_hal_sdio_query_tx_freepage(padapter, PageIdx, pxmitbuf->pg_num)) {
		...
		if (!bUpdatePageNum) {
			...
			RTW_INFO("DQWP query page, cur H:%u, M:%u, L:%u, P:%u\n"
	...
	#ifdef DBG_TX_FREE_PAGE
	RTW_INFO("DQWP write, %s:%u(len:%u)\n", sdio_tx_queue_str(PageIdx), pxmitbuf->pg_num, pxmitbuf->len);
	#endif

#ifdef DBG_SDIO_TXDMA_STATUS_DETAIL
	dump_sdio_tx_related_reg_and_check(padapter, "bwtx", PageIdx, pxmitbuf->pg_num);
#endif

	rtw_write_port(padapter, deviceId, pxmitbuf->len, (u8 *)pxmitbuf);

#ifdef DBG_SDIO_TXDMA_STATUS_DETAIL
	dump_sdio_tx_related_reg(padapter, "awtx");
#endif
