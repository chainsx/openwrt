...
#ifdef DBG_SDIO_TXDMA_STATUS_DETAIL
void dump_sdio_tx_related_reg_and_check(_adapter *adapter, const char *prefix, s8 w_page_idx, u8 w_page_num)
{
#define MAC_REG_NUM 7
	static const u32 mac_reg[MAC_REG_NUM] = {0x200, 0x204, 0x208, 0x210, 0x214, 0x418, 0x4e0};
	static const char *mac_reg_str[MAC_REG_NUM] = {"0x200", "0x204", "0x208", "0x210", "0x214", "0x418", "0x4e0"};
	u32 mac_reg_val[MAC_REG_NUM];
#define SD_REG_NUM 3
	static const u32 sd_reg[SD_REG_NUM] = {0x20, 0x24, 0x28};
	static const char *sd_reg_str[SD_REG_NUM] = {"0x20", "0x24", "0x28"};
	u32 sd_reg_val[SD_REG_NUM];

	int i;
	for (i = 0; i < MAC_REG_NUM; i++)
		mac_reg_val[i] = rtw_read32(adapter, mac_reg[i]);
	for (i = 0; i < SD_REG_NUM; i++)
		sd_reg_val[i] = SdioLocalCmd52Read4Byte(adapter, sd_reg[i]);

	RTW_PRINT("[%s] %s:0x%08x %s:0x%08x %s:0x%08x %s:0x%08x %s:0x%08x %s:0x%08x %s:0x%08x\n", prefix
		, mac_reg_str[0], mac_reg_val[0], mac_reg_str[1], mac_reg_val[1], mac_reg_str[2], mac_reg_val[2]
		, mac_reg_str[3], mac_reg_val[3], mac_reg_str[4], mac_reg_val[4], mac_reg_str[5], mac_reg_val[5]
		, mac_reg_str[6], mac_reg_val[6]);
	RTW_PRINT("[%s] %s:0x%08x %s:0x%08x %s:0x%08x\n", prefix
		, sd_reg_str[0], sd_reg_val[0], sd_reg_str[1], sd_reg_val[1], sd_reg_str[2], sd_reg_val[2]);

	if (w_page_idx >= 0) {
		u32 sd_20 = sd_reg_val[0];
		u32 sd_24 = sd_reg_val[1];
		u32 mac_204 = mac_reg_val[1];
		u32 mac_214 = mac_reg_val[4];
		u8 sd_page_num;
		u8 mac_page_num;

		switch (w_page_idx) {
		case HI_QUEUE_IDX:
			sd_page_num = (sd_20 & 0x000000FF) + ((sd_24 & 0x00FF0000) >> 16);
			mac_page_num = (mac_204 & 0x000000FF) + ((mac_204 & 0x00FF0000) >> 16);
			break;
		case MID_QUEUE_IDX:
			sd_page_num = ((sd_20 & 0x00FF0000) >> 16) + ((sd_24 & 0x00FF0000) >> 16);
			mac_page_num = ((mac_214 & 0x0000FF00) >> 8) + ((mac_204 & 0x00FF0000) >> 16);
			break;
		case LOW_QUEUE_IDX:
			sd_page_num = (sd_24 & 0x000000FF) + ((sd_24 & 0x00FF0000) >> 16);
			mac_page_num = ((mac_204 & 0x0000FF00) >> 8) + ((mac_204 & 0x00FF0000) >> 16);
			break;
		default:
			return;
		}
		rtw_warn_on(w_page_num > sd_page_num);
		rtw_warn_on(w_page_num > mac_page_num);
	}
}

void dump_sdio_tx_related_reg(_adapter *adapter, const char *prefix)
{
	dump_sdio_tx_related_reg_and_check(adapter, prefix, -1, 0);
}
#endif
