...
static void ssv6xxx_setup_ampdu_session(void *priv, struct ieee80211_sta *sta, struct sk_buff *skb)
{
    ...
    ht_cap = &sta->ht_cap;
#endif
    if ((ht_cap->ht_supported == false) || (info->flags & IEEE80211_TX_CTL_AMPDU))
        return;
    ...
    if (1 == sc->nvif) {
        if (vif_idx == 0) { // check station and enable beacon filter for vif_idx 1
            if (NULL != sc->vif_info[1].vif) {
                ...
                assoc = sc->vif_info[1].vif->cfg.assoc;
#else
                assoc = sc->vif_info[1].vif->bss_conf.assoc;
#endif
        ...
        if (vif_idx == 1) { // check station and enable beacon filter for vif_idx 0
            if (NULL != sc->vif_info[1].vif) {
#if LINUX_VERSION_CODE > KERNEL_VERSION(5, 19, 0)
                assoc = sc->vif_info[0].vif->cfg.assoc;
#else
                assoc = sc->vif_info[0].vif->bss_conf.assoc;
#endif
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(5, 19, 0)
            ssv6xxx_interface_opertaion(sc, SSV6XXX_VIF_CMD_CHG, priv_vif->vif_idx, NULL, vif->type, false, vif->cfg.assoc);
#else
...
#endif
#if LINUX_VERSION_CODE > KERNEL_VERSION(5, 18, 0)
        ptr->supp_rates = sta->deflink.supp_rates[sband->band];
#else
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(5, 18, 0)
        ptr->ht_capabilities_info = sta->deflink.ht_cap.cap;
        memcpy((void *)&ptr->supported_mcs_set[0], (const void *)&sta->deflink.ht_cap.mcs, 16);
        ptr->ht_support = sta->deflink.ht_cap.ht_supported;
#else
...
static void ssv6xxx_proc_data_rx_skb (struct ssv_softc *sc, struct sk_buff *rx_skb)
{
    ...
    if (ieee80211_is_mgmt(fc) && (sta != NULL)) {
        ...
        assoc = vif->cfg.assoc;
#else
        assoc = vif->bss_conf.assoc;
#endif
        if ((vif->type == NL80211_IFTYPE_STATION) && (assoc == false)) {
            up_read(&sc->sta_info_sem);/* END protect sta_info */
            goto drop_rx;
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(5, 19, 0)
        assoc = sc->vif_info[i].vif->cfg.assoc;
#else
        assoc = sc->vif_info[i].vif->bss_conf.assoc;
#endif
