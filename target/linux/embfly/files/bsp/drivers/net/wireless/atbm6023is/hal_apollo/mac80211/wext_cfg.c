...
#endif
	{"power_save",10,atbm_set_power_save_mode,"0:normal mode 2:power save mode"},
...
//	bool support_special=false;
	...
	if (!(ptr = (char *)atbm_kzalloc(wdata->data.length+1, GFP_KERNEL)))
		return -ENOMEM;
	atbm_printk_warn("wdata->data.length = %d %lu\n", wdata->data.length, (unsigned long)sizeof(struct BestChannelSelect));
	if ((ret = copy_from_user(ptr, wdata->data.pointer, wdata->data.length))) {
		atbm_kfree(ptr);
		return -EINVAL;
	}
...
static int atbm_ioctl_send_singleTone(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *ext)
{
	...
	atbm_printk_debug("atbm_ioctl_send_singleTone:%lu,%d\n", (unsigned long)strlen(extra), wrqu->data.length);
...
static int atbm_ioctl_associate_sta_status(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	...
	*/
	...
	if (atbm_skip_space(pos,wrqu->data.length+1 - (pos-ptr))) {
		atbm_printk_debug("%s:ptr err (%s)(%ld)\n", __func__, pos, (long)(wrqu->data.length + 1 - (pos - ptr)));
		ret = -EINVAL;
		goto exit;
	}
...
static __attribute__((unused)) void etf_PT_test_config(char *param)
{
	int Freq = 0;
	int txEvm = 0;
	int rxEvm = 0;
...
static __attribute__((unused)) int Test_FreqOffset(struct atbm_common *hw_priv, u32 *dcxo, int *pfreqErrorHz, struct rxstatus_signed *rxs_s, int channel)
{
	u8 CodeValue;
	u8 CodeValuebak;
