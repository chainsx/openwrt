...
static void dump_sdio_tx_related_reg_user(void *adapter)
{
	dump_sdio_tx_related_reg(adapter, "user");
}

static int proc_get_sdio_tx_related_reg(struct seq_file *m, void *v)
{
	struct net_device *dev = m->private;
	_adapter *adapter = (_adapter *)rtw_netdev_priv(dev);

	rtw_run_in_thread_cmd(adapter, dump_sdio_tx_related_reg_user, adapter);
...
#endif /* DBG_SDIO */
	#ifdef DBG_SDIO_TXDMA_STATUS_DETAIL
	RTW_PROC_HDL_SSEQ("sdio_tx_related_reg", proc_get_sdio_tx_related_reg, NULL),
	#endif
#endif /* CONFIG_SDIO_HCI */
