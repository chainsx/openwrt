...
minstrel_aggr_check(struct ieee80211_sta *pubsta, struct sk_buff *skb)
{
	...
	mac80211_start_tx_ba_session(pubsta, tid, 0);
}
...
static void
minstrel_calc_retransmit(struct minstrel_priv *mp,
				struct minstrel_ht_sta *mi, int index)
{
	struct minstrel_rate_stats *mrs;
...
minstrel_ht_update_rates(struct minstrel_priv *mp, struct minstrel_ht_sta *mi)
{
	...
	rates->rate[i].idx = -1;
	xrmac_rate_control_set_rates(mp->hw, mi->sta, rates);
}
...
{
	mac80211_rate_control_unregister(&mac80211_minstrel_ht);
}
