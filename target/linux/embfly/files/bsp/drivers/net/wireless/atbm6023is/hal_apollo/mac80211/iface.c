...
#endif
#ifdef P2P_MULTIVIF
		memcpy(hw->wiphy->addresses[2].addr, hw->wiphy->addresses[1].addr, ETH_ALEN);
		hw->wiphy->addresses[2].addr[4] ^= 0x80;
#endif
	...
	list_for_each_entry(nsdata, &local->interfaces, list) {
		if (nsdata != sdata && ieee80211_sdata_running(nsdata)) {
			...
			*/
			 ...
			 */
			if (atbm_compare_ether_addr(sdata->vif.addr, nsdata->vif.addr))
				continue;
...
static __attribute__((unused)) void ieee80211_sdata_uninit(struct net_device *dev)
{
	struct ieee80211_sub_if_data *sdata = IEEE80211_DEV_TO_SUB_IF(dev);
#ifdef CONFIG_ATBM_STA_LISTEN
	struct ieee80211_local *local = sdata->local;
