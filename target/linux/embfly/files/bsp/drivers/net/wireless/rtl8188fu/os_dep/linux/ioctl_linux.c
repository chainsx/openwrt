	...
	*(extra + wrqu->length) = '\0';
	...
	if (strcmp(tmp[0], "wmap") == 0) {
		...
		*extra = 0;
		...
		dev_addr_mod(padapter->pnetdev, 0, get_hal_mac_addr(padapter), ETH_ALEN);
#else
...
#if WIRELESS_EXT >= 17
static __attribute__((unused)) struct iw_statistics *rtw_get_wireless_stats(struct net_device *dev)
{
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
