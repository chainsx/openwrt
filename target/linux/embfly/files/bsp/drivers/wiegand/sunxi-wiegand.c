...
static u32 sunxi_wiegand_read(struct sunxi_wiegand *chip, u32 reg)
{
	...
	sunxi_debug(chip->dev, "%s(): read reg 0x%x = 0x%x\n", __func__, reg, val);
...
static void sunxi_wiegand_write(struct sunxi_wiegand *chip, u32 reg, u32 val)
{
	...
	sunxi_debug(chip->dev, "%s(): write reg 0x%x = 0x%x\n", __func__, reg, val);
}
...
static int wiegand_get_data_and_report(struct sunxi_wiegand *chip)
{
	...
	if (reg_val & WIEGAND_ISR_TIME_OUT) {
		sunxi_debug(chip->dev, "%s: the irq is from timeout!\n", __func__);
		return 0;
	}

	if (reg_val & WIEGAND_ISR_RECEIVE_DATA)
		sunxi_debug(chip->dev, "%s: the irq is from receive data!\n", __func__);
	...
	if (reg_check == WIEGAND_CBSR_CHECK_OK) {
		sunxi_debug(chip->dev, "%s: the odd parity and even parity ok!\n", __func__);
		input_event(chip->input_dev, EV_MSC, MSC_SCAN, reg_val);
		input_sync(chip->input_dev);
	} else if (reg_check == WIEGAND_CBSR_EVEN_FAILED) {
		sunxi_err(chip->dev, "%s: the even parity failed!\n", __func__);
		return -EPERM;
	} else if (reg_check == WIEGAND_CBSR_ODD_FAILED) {
		sunxi_err(chip->dev, "%s: the odd parity failed!\n", __func__);
		return -EPERM;
	} else if (reg_check == WIEGAND_CBSR_ODD_EVEN_FAILED) {
		sunxi_err(chip->dev, "%s: the odd parity and even parity failed\n", __func__);
		return -EPERM;
	}
...
static irqreturn_t wiegand_irq_handler(int irqnum, void *dev_id)
{
	...
	if (err)
		sunxi_err(chip->dev, "%s: the get data and report failed\n", __func__);
...
static int sunxi_wiegand_select_gpio_state(struct sunxi_wiegand *chip, char *name)
{
	...
	if (IS_ERR(pctrl_state)) {
		sunxi_err(chip->dev, "sunxi_wiegand_select_gpio_state look_state(%s) failed\n", name);
		return PTR_ERR(pctrl_state);
	}

	err = pinctrl_select_state(chip->pctrl, pctrl_state);
	if (err) {
		sunxi_err(chip->dev, "sunxi_wiegand_select_gpio_state select_state(%s) failed\n", name);
		return err;
	}
...
static int sunxi_wiegand_gpio_setup(struct sunxi_wiegand *chip)
{
	...
	if (IS_ERR(chip->pctrl)) {
		sunxi_err(chip->dev, "devm_pinctrl_get() failed\n");
...
static void sunxi_wiegand_gpio_destroy(struct sunxi_wiegand *chip)
{
	...
	if (err)
		sunxi_err(chip->dev, "%s():fail to select gpio state\n", __func__);
}
...
static int sunxi_wiegand_clk_enable(struct sunxi_wiegand *chip)
{
	...
	if (err) {
		sunxi_err(chip->dev, "Cannot enable clock 'pclk'\n");
		goto err0;
	}

	err = clk_prepare_enable(chip->wclk);
	if (err) {
		sunxi_err(chip->dev, "Cannot enable clock 'wclk'\n");
		goto err1;
	}
...
static int sunxi_wiegand_clk_setup(struct sunxi_wiegand *chip)
{
	...
	if (IS_ERR(chip->pclk)) {
		sunxi_err(chip->dev, "Failed to get clock 'pclk'\n");
		return PTR_ERR(chip->pclk);
	}

	chip->wclk = devm_clk_get(chip->dev, "wclk");
	if (IS_ERR(chip->wclk)) {
		sunxi_err(chip->dev, "Failed to get clock 'wclk'\n");
...
static int sunxi_wiegand_hwinit(struct platform_device *pdev, struct sunxi_wiegand *chip)
{
	...
	if (err) {
		sunxi_err(dev, "%s: get the protocol-type failed!\n", __func__);
		return err;
	}
	sunxi_debug(dev, "%s: the protocol-type is %d (0:26bit, 1:34bit)\n", __func__, config->protocol_type);
	...
	if (err) {
		sunxi_err(dev, "%s: get high-parity-polar failed!\n", __func__);
		return err;
	}
	...
	sunxi_debug(dev, "%s: the high-parity-polar is %d (0:even check 1:odd check)\n", __func__, config->high_parity_polar);

	err = of_property_read_u32(np, "low-parity-polar", &config->low_parity_polar);
	if (err) {
		sunxi_err(dev, "%s: get low-parity-polar failed!\n", __func__);
		return err;
	}
	reg_val = sunxi_wiegand_read(chip, WIEGAND_CBP);
	reg_val &= ~(BIT(0));
	...
	sunxi_wiegand_write(chip, WIEGAND_CBP, reg_val);
	sunxi_debug(dev, "%s: the low-parity-polar is %d (0:even check 1:odd check) \n", __func__, config->low_parity_polar);

	err = of_property_read_u32(np, "data-polar", &config->data_polar);
	if (err) {
		sunxi_err(dev, "%s: get data-polar failed!\n", __func__);
		return err;
	}
	...
	sunxi_debug(dev, "%s: the data-polar is %d (0:normal, 1:inverse)\n", __func__, config->data_polar);

	err = of_property_read_u32(np, "clock-div", &config->clock_div);
	if (err) {
		sunxi_err(dev, "%s: get clock-div failed!\n", __func__);
		return err;
	}
	sunxi_debug(dev, "%s: the clock-div is %d \n", __func__, config->clock_div);
...
static int sunxi_wiegand_input_setup(struct sunxi_wiegand *chip)
{
	...
	if (err) {
		sunxi_err(chip->dev, "wiegand input register failed\n");
		goto err0;
	}
...
static int sunxi_wiegand_probe(struct platform_device *pdev)
{
	...
	if (!chip) {
		sunxi_err(dev, "fail to alloc memery\n");
		return -ENOMEM;
	}
	...
	if (err) {
		sunxi_err(dev, "misc_register() failed!\n");
		goto err0;
	}

	err = sunxi_wiegand_input_setup(chip);
	if (err) {
		sunxi_err(dev, "sunxi_wiegand_input_setup() failed\n");
		goto err1;
	}

	err = sunxi_wiegand_clk_setup(chip);
	if (err) {
		sunxi_err(dev, "sunxi_wiegand_clk_setup() failed!\n");
		goto err2;
	}

	err = sunxi_wiegand_gpio_setup(chip);
	if (err) {
		sunxi_err(dev, "sunxi_wiegand_gpio_setup() failed!\n");
		goto err3;
	}

	err = sunxi_wiegand_hwinit(pdev, chip);
	if (err) {
		sunxi_err(dev, "sunxi_wiegand_hwinit() failed\n");
		goto err4;
	}

	err = sunxi_wiegand_irq_setup(pdev, chip);
	if (err) {
		sunxi_err(dev, "sunxi_wiegand_irq_setup() failed!\n");
		goto err4;
	}
...
static int sunxi_wiegand_suspend(struct device *dev)
{
	...
	sunxi_debug(dev, "sunxi_wiegand_suspend\n");
...
static int sunxi_wiegand_resume(struct device *dev)
{
	...
	sunxi_debug(dev, "sunxi_wiegand_resume\n");
