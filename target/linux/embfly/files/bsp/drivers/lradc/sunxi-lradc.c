...
static irqreturn_t sunxi_lradc_irq_handler(int irq, void *dummy)
{
	...
	if (reg_val & SUNXI_LRADC_ADC0_DATAPEND) {
		...
		if (chip->key_cnt >= chip->filter_cnt) {
			sunxi_debug(chip->dev, "vol_port is %d mv\n", chip->compare_before * chip->hw_data->resol);
	...
	if (reg_val & SUNXI_LRADC_ADC0_UPPEND) {
		...
		for (i = 0; i < SUNXI_LRADC_MAX_KEYPRESS; i++) {
			...
			if (key > 0) {
				sunxi_debug(chip->dev, "report : %d, key : %d\n", chip->before_code, key);
				input_report_key(chip->input_dev, key, 0);
				input_sync(chip->input_dev);
			}
		}
		sunxi_debug(chip->dev, "key up\n");
...
static int sunxi_lradc_key_setup(struct sunxi_lradc *chip)
{
	...
	if (chip->key_num < 1 || chip->key_num >= SUNXI_LRADC_VOL_NUM) {
		sunxi_err(chip->dev, "incorrect key number.\n");
		return -EINVAL;
	}
...
static int sunxi_lradc_resource_get(struct sunxi_lradc *chip)
{
	...
	if (!chip->key_debounce)
		sunxi_warn(chip->dev, "get key_debounce failed, please check whether to enable key debounce\n");
	else {
		err = of_property_read_u32(chip->np, "debounce_value", &debounce_value);
		if (err) {
			sunxi_err(chip->dev, "get debounce_value failed\n");
	...
	if (!chip->res) {
		sunxi_err(chip->dev, "get resource failed\n");
		return -ENXIO;
	}

	chip->reg_base = devm_ioremap_resource(chip->dev, chip->res);
	if (IS_ERR(chip->reg_base)) {
		sunxi_err(chip->dev, "get ioremap resource failed\n");
	...
	if (of_property_read_u32(chip->np, "filter-cnt", &chip->filter_cnt)) {
		sunxi_warn(chip->dev, "warn: filter_cnt not set, default value is 3\n");
		chip->filter_cnt = 3;
	}

	err = devm_request_irq(chip->dev, chip->irq_num, sunxi_lradc_irq_handler, 0, "sunxikbd", chip);
	if (err) {
		sunxi_err(chip->dev, "request irq failed\n");
		return err;
	}

	err = sunxi_lradc_key_setup(chip);
	if (err) {
		sunxi_err(chip->dev, "key init failed\n");
		return err;
	}
	...
	if (chip->jack_detect) {
		if (of_property_read_u32(chip->np, "jack-detect", &chip->lradc_chan)) {
			sunxi_debug(chip->dev, "jack-detect lradc_chan not set, default value is 0\n");
			chip->lradc_chan = 0;
		}
		sunxi_debug(chip->dev, "lradc supports jack detection.\n");
	}
...
static int sunxi_lradc_clk_enable(struct sunxi_lradc *chip)
{
	...
	if (err) {
		sunxi_err(chip->dev, "reset_control_deassert failed!\n");
		return err;
	}

	err = clk_prepare_enable(chip->mclk);
	if (err) {
		sunxi_err(chip->dev, "clock enable failed\n");
		goto err0;
	}
...
static int sunxi_lradc_hw_init(struct sunxi_lradc *chip)
{
	...
	if (chip->hw_data->has_clock) {
		...
		if (err) {
			sunxi_err(chip->dev, "init lradc clock failed\n");
			return err;
		}
	}
...
static int sunxi_lradc_inputdev_register(struct sunxi_lradc *chip)
{
	...
	if (!chip->input_dev) {
		sunxi_err(chip->dev, "input allocate device failed\n");
		return -ENOMEM;
	}
	...
	sunxi_info(chip->dev, ("support report repeat key value\n");
#else
	...
	if (err) {
		sunxi_err(chip->dev, "register input device failed\n");
		return err;
	}
...
static void sunxi_lradc_remove_iio(void *_data)
{
	...
	if (IS_ERR_OR_NULL(indio_dev)) {
		sunxi_err(NULL, "indio_dev is null\n");
	} else {
...
static int sunxi_lradc_probe(struct platform_device *pdev)
{
	...
	if (err) {
		sunxi_err(chip->dev, "sunxi lradc get resource failed\n");
		goto err0;
	}
	...
	if (chip->jack_detect) {
		...
		if (IS_ERR(chip->edev)) {
			sunxi_err(chip->dev, "failed to allocate extcon device\n");
			return -ENOMEM;
		}

		err = devm_extcon_dev_register(chip->dev, chip->edev);
		if (err < 0) {
			sunxi_err(chip->dev, "failed to register extcon device\n");
			return err;
		}
	...
	if (err) {
		sunxi_err(chip->dev, "sunxi lradc inputdev register failed\n");
		goto err1;
	}

	err = sunxi_lradc_hw_init(chip);
	if (err) {
		sunxi_err(chip->dev, "sunxi lradc hw_init failed\n");
		goto err2;
	}
...
#if IS_ENABLED(CONFIG_IIO)
	err = sunxi_lradc_iio_init(pdev);
	if (err) {
		sunxi_err(chip->dev, "sunxi lradc iio_init failed\n");
		goto err2;
	}
#endif

	sunxi_info(chip->dev, "sunxi lradc init success\n");
	return 0;
...
static int sunxi_lradc_suspend(struct device *dev)
{
	...
	sunxi_debug(chip->dev, "[%s] enter standby\n", __func__);
...
static int sunxi_lradc_resume(struct device *dev)
{
	...
	sunxi_debug(chip->dev, "[%s] return from standby\n", __func__);
