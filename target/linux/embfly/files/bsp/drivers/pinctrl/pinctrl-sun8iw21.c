...
#define SUNXI_PINCTRL_VERSION		"1.1.0"
...
static const struct sunxi_desc_pin sun8iw21p1_pins[] = {
...
#else /* !IS_ENABLED(CONFIG_AW_FPGA_BOARD) */
	...
	SUNXI_PIN(SUNXI_PINCTRL_PIN(I, 4),
		...
		SUNXI_FUNCTION(0xf, "io_disabled")),	/* io_disabled */
#endif /* IS_ENABLED(CONFIG_AW_FPGA_BOARD) */
};
...
static int sun8iw21p1_pinctrl_suspend_noirq(struct device *dev)
{
...
static int sun8iw21p1_pinctrl_resume_noirq(struct device *dev)
{
	struct sunxi_pinctrl *pctl = dev_get_drvdata(dev);
	unsigned long flags;
...
#define PINCTRL_PM_OPS	(&sun8iw21p1_pinctrl_pm_ops)
#else /* !IS_ENABLED(CONFIG_PM_SLEEP) */
static int pinctrl_pm_alloc_mem(struct platform_device *pdev)
{
	return 0;
}
...
static int sun8iw21p1_pinctrl_probe(struct platform_device *pdev)
{
	...
	sunxi_info(&pdev->dev, "sunxi pinctrl version: %s\n", SUNXI_PINCTRL_VERSION);
