...
static void spi_set_all_burst_received(void __iomem *base_addr, bool dhb)
{
	u32 reg_val = readl(base_addr+SPI_TC_REG);
...
static int sunxi_spi_mode_check(struct sunxi_spi *sspi, struct spi_device *spi, struct spi_transfer *t)
{
	...
	if (t->tx_buf && t->rx_buf) {
		spi_set_all_burst_received(sspi->base_addr, false);
		...
		spi_set_all_burst_received(sspi->base_addr, true);
			    ...
			    struct spi_transfer *t, struct spi_transfer *t2)
{
	...
	dprintk(DEBUG_INFO, "[spi%d] begin transfer two, txbuf %p, rxbuf %p, txlen %d, rxlen %d\n",
		spi->master->bus_num, t->tx_buf, t2->rx_buf, t->len, t2->len);
...
static int sunxi_spi_probe(struct platform_device *pdev)
{
	...
	master->handle_err		= sunxi_spi_handle_err;
