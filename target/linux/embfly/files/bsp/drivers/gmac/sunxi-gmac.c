...
#define SUNXI_GMAC_MODULE_VERSION	"2.5.4"
...
static irqreturn_t sunxi_gmac_interrupt(int irq, void *dev_id)
{
	...
	if (likely(status == handle_tx_rx))
		...
		sunxi_info(chip->dev, "Do nothing.....\n");
...
static void sunxi_gmac_tx_complete(struct sunxi_gmac *chip)
{
	...
	spin_lock_irqsave(&chip->tx_lock, flags);
	...
	spin_unlock_irqrestore(&chip->tx_lock, flags);
}
...
static netdev_tx_t sunxi_gmac_xmit(struct sk_buff *skb, struct net_device *ndev)
{
	...
	sunxi_debug(chip->dev, "TX descripotor DMA: 0x%08x, dirty: %d, clean: %d\n",
