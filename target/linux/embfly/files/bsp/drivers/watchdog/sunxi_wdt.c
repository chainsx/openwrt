...
static int sunxi_wdt_get_resource(struct sunxi_wdt_dev *sunxi_wdt)
{
	struct device *dev = sunxi_wdt->wdt_dev.parent;
	struct device_node *np = dev->of_node;
	const char *key;
	int ret;

	key = "wdt_keep_reg";
	ret = of_property_read_u32(np, key, &sunxi_wdt->wdt_keep_reg);
	if (ret) {
		dev_dbg(dev, "%s not found\n", key);
		sunxi_wdt->wdt_keep_reg = 0;
	}

	key = "kernel,autostart";
	sunxi_wdt->auto_start = of_property_read_bool(np, key);
	if (!sunxi_wdt->auto_start) {
		dev_dbg(dev, "%s not found\n", key);
		goto out;
	}

	key = "wdt_keep_reg";
	ret = of_property_read_u32(np, "timeout-sec", &sunxi_wdt->timeout_sec);
	if (ret) {
		dev_dbg(dev, "%s not found\n", key);
		sunxi_wdt->timeout_sec = WDT_MAX_TIMEOUT;
	}
	if (sunxi_wdt->timeout_sec == 0)
		sunxi_wdt->timeout_sec = WDT_MAX_TIMEOUT;
out:
	return 0;
}
...
static int sunxi_wdt_probe(struct platform_device *pdev)
{
	...
	sunxi_wdt_get_resource(sunxi_wdt);
	...
	if (!sunxi_wdt->wdt_keep_reg)
		sunxi_wdt_stop(&sunxi_wdt->wdt_dev);
	else
		dev_info(dev, "sunxi wdt keep config\n");

	if (sunxi_wdt->auto_start) {
		sunxi_wdt_set_timeout(&sunxi_wdt->wdt_dev, sunxi_wdt->timeout_sec);
		sunxi_wdt_start(&sunxi_wdt->wdt_dev);
		sunxi_wdt_ping(&sunxi_wdt->wdt_dev);
		dev_info(dev, "sunxi wdt auto started (%ds timeout)\n", sunxi_wdt->timeout_sec);
	}
