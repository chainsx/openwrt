...
#if defined(SUPPORT_INDEPENDENT_DEVICE_COMMON)
static volatile struct __de_lcd_top_dev_t *lcd_top[2];
#else
...
#if IS_ENABLED(CONFIG_ARCH_SUN60IW2) && defined(SUPPORT_INDEPENDENT_DEVICE_COMMON)
	if (sel == 3) {
		lcd_top[1]->tcon_tv_setup.bits.tv0_clk_src = 0;
		lcd_top[1]->tcon_clk_gate.bits.tv0_clk_gate = en & 0x1;
	} else if (sel == 4) {
		lcd_top[1]->tcon_tv_setup.bits.tv1_clk_src = TV_CLK_F_TVE;
		lcd_top[1]->tcon_clk_gate.bits.tv1_clk_gate = en & 0x1;
	}
#else
	if (sel == 2) {
...
s32 tcon1_edp_clk_enable(u32 sel, u32 en)
{
	...
	if (en) {
		lcd_top[1]->tcon_tv_setup.bits.tv1_clk_src = TV_CLK_F_TVE;
		lcd_top[1]->tcon_clk_gate.bits.tv1_clk_gate = TV_CLK_F_TVE;
	} else {
		lcd_top[1]->tcon_tv_setup.bits.tv1_clk_src = 0;
		lcd_top[1]->tcon_clk_gate.bits.tv1_clk_gate = 0;
	}
#else
	if (en) {
		lcd_top[0]->tcon_tv_setup.bits.tv1_clk_src = TV_CLK_F_TVE;
...
#if IS_ENABLED(CONFIG_ARCH_SUN60IW2) && defined(SUPPORT_INDEPENDENT_DEVICE_COMMON)
	if (sel == 3)
		lcd_top[1]->tcon_clk_gate.bits.tv0_clk_gate = en;

	if (en) {
		lcd_top[1]->tcon_clk_gate.bits.hdmi_src = 1;
	} else {
		/* disable tcon output to hdmi */
		if (((sel == 3) && (lcd_top[1]->tcon_clk_gate.bits.hdmi_src == 1))) {
			lcd_top[1]->tcon_clk_gate.bits.hdmi_src = 0;
		}
	}
#else
	if (sel == 2)
...
s32 tcon0_dsi_clk_source(u32 sel, struct disp_panel_para *panel)
{
	u32 en = 1;
...
static s32 tcon0_cfg_mode_tri(u32 sel, struct disp_panel_para *panel)
{
	...
	if (panel->lcd_start_delay != 0)
		...
		DE_WARN("vbp is too small, please readjust the timing parameters to increase vbp\n");
...
{
	de_gamma_set_table(extend_panel->mgr_id, 0, DISP_GAMMA_TYPE, extend_panel->lcd_gamma_en,
