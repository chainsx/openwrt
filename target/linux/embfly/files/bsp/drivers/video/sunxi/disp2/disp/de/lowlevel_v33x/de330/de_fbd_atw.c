...
struct de_fbd_dump_regs_priv {
	struct dump_regs_module *mod;
};

struct de_fbd_dump_regs_priv dr_fbd_priv;
	...
	struct de_fbd_info *info)
{
	...
	default:
		...
		info->compbits[3] = 8;
		info->yuv_tran = 1;
		DE_WARN("no support afbd header layout %d\n", hd->header_layout);
		break;
	}
	return 0;
}
...
static void __iomem *fbd_get_io_regs(struct de_fbd_atw_private *priv)
{
	if (priv->type == FBD_ATW_TYPE_VI) {
		return (void *)(priv->fbd_atw_v_blks[0].reg_addr);
	} else if (priv->type == FBD_TYPE_UI) {
		return (void *)(priv->fbd_u_blks[0].reg_addr);
	}

	return NULL;
}

static int de_fbd_get_enable_info(const struct dump_regs_user_setting *setting, char *buf)
{

	return 0;
}

static int de_fbd_get_enable_status(const struct dump_regs_user_setting *setting)
{
	int disp = setting->index.disp;
	int chn = setting->index.chn;
	struct de_fbd_atw_private *priv = &(fbd_atw_priv[disp][chn]);
	void __iomem *regs = fbd_get_io_regs(priv);
	struct fbd_u_reg *ureg;
	struct fbd_atw_v_reg *vreg;
	u32 value = 0;
	u32 enable;

	if (priv->type == FBD_ATW_TYPE_VI) {
		ureg = (struct fbd_u_reg *)regs;
		de_lowlevel_ioread(&(ureg->fbd_ctl), (void *)&value, 4);
		enable = ((union fbd_v_ctl_reg)value).bits.en & 0x1;
	} else if (priv->type == FBD_TYPE_UI) {
		vreg = (struct fbd_atw_v_reg *)regs;
		de_lowlevel_ioread(&(vreg->fbd_ctl), (void *)&value, 4);
		enable = ((union fbd_u_ctl_reg)value).bits.fbd_en & 0x1;
	} else {
		enable = 0;
		DUMP_REGS_PRINT("fbd: not support");
	}
	DUMP_REGS_PRINT("fbd: disp%d chn%d enable%d\n", disp, chn, enable);

	return enable;
}

static int de_fbd_dump_by_user_setting(const struct dump_regs_user_setting *setting, char *buf)
{
	unsigned int num = 0;
	struct de_fbd_atw_private *priv;
	int i, j, disp, chn, screen_num, chn_num, is_enable, is_support;

	num += sprintf(buf + num, "%s module:\n\n", dr_fbd_priv.mod->name);

	if (setting->use_index_find) {
		disp = setting->index.disp;
		chn = setting->index.chn;
		priv = &(fbd_atw_priv[disp][chn]);
		is_enable = de_fbd_get_enable_status(setting);
		is_support = de_feat_is_support_fbd_by_layer(disp, chn, 0);
		...
		if ((is_enable || setting->force_dump) && is_support)
			num += do_dump_regs_block(dr_fbd_priv.mod, priv->reg_blks, ARRAY_SIZE(priv->reg_blks), buf + num);
		num += sprintf(buf + num, "\n");
	} else {
		...
		for (i = 0; i < screen_num; i++) {
			...
			for (j = 0; j < chn_num; j++) {
				...
				priv = &(fbd_atw_priv[i][j]);
				is_enable = de_fbd_get_enable_status(&msetting);
				is_support = de_feat_is_support_fbd_by_layer(i, j, 0);
...
int de_fbd_register_dr_module(void)
{
#define FBD_MAX_BLOCK_SIZE (0x58)
	struct dump_regs_module *mod = dr_module_alloc(FBD_MAX_BLOCK_SIZE);

	dr_fbd_priv.mod = mod;

	mod->type = DISP_FBD_MODULE;
	strcpy(mod->name, module_name[mod->type]);
	mod->ops.dump_by_user_setting = de_fbd_dump_by_user_setting;
	mod->ops.get_enable_info = de_fbd_get_enable_info;
	mod->ops.get_enable_status = de_fbd_get_enable_status;
	register_dr_module(mod);

	return 0;
}

int de_fbd_unregister_dr_module(void)
{
	unregister_dr_module(dr_fbd_priv.mod);
	return 0;
}
