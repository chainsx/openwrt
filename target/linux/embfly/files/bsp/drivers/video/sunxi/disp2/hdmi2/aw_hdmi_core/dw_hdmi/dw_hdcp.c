...
static void _dw_hdcp14_start_auth(struct work_struct *work)
{
	...
	/* TODO: Because CEC uses the interrupt handler function,
	 * if the mask here is turned on, it will cause problems.
	 * In the future, if the interrupt is indeed needed,
	 * it will be moved to the interrupt handler function for processing.
	 */
...
ssize_t dw_hdcp_config_dump(dw_hdmi_dev_t *dev, char *buf)
{
	...
	if (hdcp22_enable) {
		...
		n += sprintf(buf + n, " - [hdcp22]\n");
		n += sprintf(buf + n, "    - auth state: esm enable failed!\n");
	}
#endif
	return n;
}
