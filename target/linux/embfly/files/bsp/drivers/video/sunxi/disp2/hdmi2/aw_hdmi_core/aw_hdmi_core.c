...
static struct disp_hdmi_mode aw_hdmi_mode_tbl[] = {
	...
	/* VESA Timing Start */
	{DISP_VESA_MOD_640_480P_60,       HDMI_640x480P60,           },
	{DISP_VESA_MOD_800_600P_56,       HDMI_800x600P56,           },
	{DISP_VESA_MOD_800_600P_60,       HDMI_800x600P60,           },
	{DISP_VESA_MOD_1024_768P_60,      HDMI_1024x768P60,          },
	{DISP_VESA_MOD_1024_768P_70,      HDMI_1024x768P70,          },
	{DISP_VESA_MOD_1152_864P_75,      HDMI_1152x864P75_4x3,      },
	{DISP_VESA_MOD_1280_768P_60,      HDMI_1280x768P60_5x4,      },
	{DISP_VESA_MOD_1280_800P_60,      HDMI_1280x800P60_16x10,    },
	{DISP_VESA_MOD_1280_960P_60,      HDMI_1280x960P60_4x3,      },
	{DISP_VESA_MOD_1280_1024P_60,     HDMI_1280x1024P60_5x4,     },
	{DISP_VESA_MOD_1360_768P_60,      HDMI_1360x768P60_16x9,     },
	{DISP_VESA_MOD_1366_768P_60,      HDMI_1366x768P60_16x9,     },
	{DISP_VESA_MOD_1400_1050P_60,     HDMI_1400x1050P60_4x3,     },
	{DISP_VESA_MOD_1440_900P_60,      HDMI_1440x900P60_16x10,    },
	{DISP_VESA_MOD_1600_900P_60,      HDMI_1600x900P60_16x9,     },
	{DISP_VESA_MOD_1680_1050P_60,     HDMI_1680x1050P60_16x10,   },
	{DISP_VESA_MOD_1920_1200P_60,     HDMI_1920x1200P60_16x10,   },
	{DISP_VESA_MOD_1920_1440P_60,     HDMI_1920x1440P60_4x3,     },
	/* VESA Timing End */
};
...
static void _phy_core_irq_all_mask(void)
{
	dw_phy_interrupt_all_mask(g_dw_dev);
}

static u32 _phy_core_get_ir_state(void)
{
	return dw_phy_get_ir_state(g_dw_dev);
}

static void _phy_core_clear_ir_state(unsigned state)
{
	dw_phy_clear_ir_state(g_dw_dev, state);
}
...
static ssize_t _audio_core_config_dump(char *buf)
{
	ssize_t n = 0;

	dw_audio_param_t *pAudio = &g_aw_core->mode.pAudio;

	char *_aud_interface[] = {"I2S", "SPDIF", "HBR", "GPA", "DMA"};
	char *_aud_coding[] = { "NULL", "PCM", "AC3", "MPEG1", "MP3", "MPEG2",
		"AAC", "DTS", "ATRAC", "ONE_BIT_AUDIO", "DOLBY_DIGITAL_PLUS",
		"DTS_HD", "MAT", "DST", "WMAPRO"};

	if (!pAudio) {
		hdmi_err("%s: pAudio is null!!!\n", __func__);
		return 0;
	}

	if (!g_dw_dev) {
		hdmi_err("%s: g_dw_dev is null!!!\n", __func__);
		return 0;
	}

	n += sprintf(buf + n, "\n[audio core level]\n");
	n += sprintf(buf + n, " - audio_on: %d\n", g_dw_dev->snps_hdmi_ctrl.audio_on);
	if (pAudio->mInterfaceType < 0)
		n += sprintf(buf + n, " - audio interface type[%d] invalid!", pAudio->mInterfaceType);
	else {
		n += sprintf(buf + n, " - interface: %s, coding: %s\n",
				_aud_interface[pAudio->mInterfaceType], _aud_coding[pAudio->mCodingType]);
		n += sprintf(buf + n, " - sample freq: %dHz, sample size: %d\n",\
				pAudio->mSamplingFrequency, pAudio->mSampleSize);
		n += sprintf(buf + n, " - channel alloc[%d], channel num[%d], clock factor[%d]\n", \
				pAudio->mChannelAllocation, pAudio->mChannelNum, pAudio->mClockFsFactor);
	}

	n += dw_audio_dump(g_dw_dev, buf + n);
...
void _video_core_show_pattern(u8 enable, u32 value)
{
	dw_fc_video_force_value(g_dw_dev, enable, value);
}
...
u32 _video_core_get_vic_code_sw(void)
{
	return g_aw_core->mode.pVideo.mDtd.mCode;
}
...
int _video_core_set_vsif_config(void *config, struct disp_device_dynamic_config *scfg)
{
	return dw_hdr10p_configure(g_dw_dev, config,
			&g_aw_core->mode.pVideo,
			&g_aw_core->mode.pProduct, scfg);
}
...
static int _edid_sink_supports_vic_code(u32 vic_code)
{
	...
	/* check estable timing */
	data  = edid_block0->established_timings.t1;
	data |= edid_block0->established_timings.t2 << 8;
	data |= edid_block0->established_timings.mfg_rsvd << 16;
	if (vic_code >= HDMI_ESTABLE_BASE && vic_code <= HDMI_ESTABLE_MAX) {
		for (i = 0; i < 24; i++) {
			if (((data >> i) & 0x1) &&
					((vic_code - HDMI_ESTABLE_BASE) == i)) {
				hdmi_inf("check support vic %d in estable bit %d\n", vic_code, i);
				return true;
			}
		}
	}

	for (i = 0; (i < edid_exten->edid_mStandardIndex) && (edid_exten->edid_mStandard[i].code != 0); i++) {
		if (edid_exten->edid_mStandard[i].code == vic_code)
			return true;
	}
	...
	if ((vic_code == HDMI_720P_50_3D_FP) ||
			(vic_code == HDMI_720P_60_3D_FP) ||
			(vic_code == HDMI_1080P_24_3D_FP)) {
	...
	/* check edid sace dtd timing table */
	for (i = 0; i < edid_exten->edid_mDtdIndex; i++) {
		if (dtd.mPixelClock != edid_exten->edid_mDtd[i].mPixelClock)
			continue;
		if (dtd.mVActive != edid_exten->edid_mDtd[i].mVActive)
			continue;
		if (dtd.mHActive != edid_exten->edid_mDtd[i].mHActive)
			continue;
		if (dtd.mVBlanking != edid_exten->edid_mDtd[i].mVBlanking)
			continue;
		if (dtd.mHBlanking != edid_exten->edid_mDtd[i].mHBlanking)
			continue;
		hdmi_inf("vic %d is match in dtd table %d: %dx%d\n",
			vic_code, i, dtd.mHActive, dtd.mVActive);
		return true;
	}
...
static int _aw_core_main_close(void)
{
	...
	memset(product, 0x0, sizeof(dw_product_param_t));
	return true;
}
...
void _aw_core_irq_all_mute(u8 mute)
{
	if (mute)
		dw_mc_irq_mask_all(g_dw_dev);
	else
...
int _aw_core_init_ops_device(void)
{
	...
	g_aw_core->dev_ops.dev_irq_all_mute          = _aw_core_irq_all_mute;
...
int _aw_core_init_ops_video(void)
{
	...
	g_aw_core->video_ops.get_vic_code_sw     = _video_core_get_vic_code_sw;
	...
	g_aw_core->video_ops.show_pattern        = _video_core_show_pattern;
	g_aw_core->video_ops.set_vsif_config     = _video_core_set_vsif_config;
	return 0;
}
...
int _aw_core_init_ops_audio(void)
{
	...
	g_aw_core->audio_ops.audio_dump         = _audio_core_config_dump;
	return 0;
}
...
int _aw_core_init_ops_phy(void)
{
	...
	g_aw_core->phy_ops.irq_all_mask      = _phy_core_irq_all_mask;
	g_aw_core->phy_ops.get_ir_state      = _phy_core_get_ir_state;
	g_aw_core->phy_ops.clear_ir_state    = _phy_core_clear_ir_state;
