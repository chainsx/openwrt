...
struct de_ovl_dump_regs_priv {
	struct dump_regs_module *mod;
};

struct de_ovl_dump_regs_priv dr_ovl_priv;
	...
	struct de_ovl_private *priv, u32 blk_id, u32 dirty)
{
	if (priv->reg_blks[blk_id].rcq_hd) {
		...
		DE_WARN("rcq_head is null ! ovl_type=%d, blk_id=%d\n",
			priv->type, blk_id);
	}
}
...
static void __iomem *get_ovl_io_regs(struct de_ovl_private *priv)
{
	if (priv->type == OVL_TYPE_VI) {
		return (void *)(priv->v_reg_blks[0].reg_addr);
	} else if (priv->type == OVL_TYPE_UI) {
		return (void *)(priv->u_reg_blks[0].reg_addr);
	}

	return NULL;
}

static int de_ovl_get_enable_info(const struct dump_regs_user_setting *setting, char *buf)
{

	return 0;
}

static int de_ovl_get_enable_status(const struct dump_regs_user_setting *setting)
{
	int disp = setting->index.disp;
	int chn = setting->index.chn;
	struct de_ovl_private *priv = &(ovl_priv[disp][chn]);
	void __iomem *regs = get_ovl_io_regs(priv);
	struct ovl_v_reg *vreg;
	struct ovl_u_reg *ureg;
	u32 value[4] = {0};
	u32 enable = 0;

	if (priv->type == OVL_TYPE_VI) {
		vreg = (struct ovl_v_reg *)regs;
		de_lowlevel_ioread(&(vreg->lay[0].ctl), (void *)&value[0], 4);
		de_lowlevel_ioread(&(vreg->lay[1].ctl), (void *)&value[1], 4);
		de_lowlevel_ioread(&(vreg->lay[2].ctl), (void *)&value[2], 4);
		de_lowlevel_ioread(&(vreg->lay[3].ctl), (void *)&value[3], 4);
		enable = ((union ovl_v_attctl_reg)value[0]).bits.en | ((union ovl_v_attctl_reg)value[1]).bits.en |
			 ((union ovl_v_attctl_reg)value[2]).bits.en | ((union ovl_v_attctl_reg)value[3]).bits.en;
	} else if (priv->type == OVL_TYPE_UI) {
		ureg = (struct ovl_u_reg *)regs;
		de_lowlevel_ioread(&(ureg->lay[0].ctl), (void *)&value[0], 4);
		de_lowlevel_ioread(&(ureg->lay[1].ctl), (void *)&value[1], 4);
		de_lowlevel_ioread(&(ureg->lay[2].ctl), (void *)&value[2], 4);
		de_lowlevel_ioread(&(ureg->lay[3].ctl), (void *)&value[3], 4);
		enable = ((union ovl_u_attctl_reg)value[0]).bits.en | ((union ovl_u_attctl_reg)value[1]).bits.en |
			 ((union ovl_u_attctl_reg)value[2]).bits.en | ((union ovl_u_attctl_reg)value[3]).bits.en;
	}

	DUMP_REGS_PRINT("ovl: disp%d chn%d enable%d\n", disp, chn, enable);

	return enable;
}

static int de_ovl_dump_by_user_setting(const struct dump_regs_user_setting *setting, char *buf)
{
	unsigned int num = 0;
	struct de_ovl_private *priv;
	int i, j, disp, chn, screen_num, chn_num, is_enable;

	num += sprintf(buf + num, "%s module:\n\n", dr_ovl_priv.mod->name);

	if (setting->use_index_find) {
		disp = setting->index.disp;
		chn = setting->index.chn;
		priv = &(ovl_priv[disp][chn]);
		is_enable = de_ovl_get_enable_status(setting);

		num += sprintf(buf + num, "index(%d, %d) (%s)\n", disp, chn, is_enable ? "enabled" : "disabled");
		if (is_enable || setting->force_dump)
			num += do_dump_regs_block(dr_ovl_priv.mod, priv->reg_blks, ARRAY_SIZE(priv->reg_blks), buf + num);
		num += sprintf(buf + num, "\n");
	} else {
		...
		for (i = 0; i < screen_num; i++) {
			...
			for (j = 0; j < chn_num; j++) {
				...
				priv = &(ovl_priv[i][j]);
				is_enable = de_ovl_get_enable_status(&msetting);
...
int de_ovl_register_dr_module(void)
{
#define OVL_MAX_BLOCK_SIZE (0x60)
	struct dump_regs_module *mod = dr_module_alloc(OVL_MAX_BLOCK_SIZE);

	dr_ovl_priv.mod = mod;

	mod->type = DISP_OVL_MODULE;
	strcpy(mod->name, module_name[mod->type]);
	mod->ops.dump_by_user_setting = de_ovl_dump_by_user_setting;
	mod->ops.get_enable_info = de_ovl_get_enable_info;
	mod->ops.get_enable_status = de_ovl_get_enable_status;
	register_dr_module(mod);

	return 0;
}

int de_ovl_unregister_dr_module(void)
{
	unregister_dr_module(dr_ovl_priv.mod);
	return 0;
}
