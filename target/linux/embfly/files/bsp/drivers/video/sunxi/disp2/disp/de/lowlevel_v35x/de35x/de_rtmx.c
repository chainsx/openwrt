...
static s32 de_rtmx_init_rcq(u32 disp)
{
	...
	if (rcq_info->reg_blk == NULL) {
		DE_WARN("kalloc for de_reg_block failed\n");
		return -1;
	}
...
s32 de_rtmx_start(u32 disp)
{
	...
	de_top_set_ahb_read_mode(1); /* Conflict Mode */
...
s32 de_rtmx_mgr_apply(u32 disp, struct disp_manager_data *data)
{
	...
	if (data->flag & MANAGER_ENABLE_DIRTY) {
		...
		ctx->output.eotf = (enum de_eotf)data->config.eotf;
		ctx->output.data_bits = (enum de_data_bits)data->config.data_bits;
		ctx->output.hdr_type = (enum disp_hdr_type)data->config.hdr_type;
...
static u32 de_rtmx_chn_fix_size_for_ability(u32 disp, u32 chn)
{
	...
	*/
	...
	DE_INFO("de_freq_hz=%lld, tmp=%d,vsu_outh=%d,ovl_h=%d\n",
		ctx->clk_rate_hz, tmp, chn_info->scn_win.height,
		chn_info->ovl_out_win.height);
	DE_INFO("ability: %lld,%lld\n",
...
#if defined(SUPPORT_SHARP_RGB_PQ)
	if (de_feat_is_support_vep_by_chn(disp, chn)) {
		de_sharp_set_csc(disp, chn, &icsc_info, &ocsc_info);

		/* convert channel RGB to YUV data */
		icsc_info.px_fmt_space = chn_info->px_fmt_space = ocsc_info.px_fmt_space;
		icsc_info.color_space = chn_info->color_space = ocsc_info.color_space;
		icsc_info.color_range = chn_info->color_range = ocsc_info.color_range;
	}
#endif
			...
			unsigned int *trd_addr, unsigned long long *lay_laddr)
{
	...
	struct disp_rectsz actual_size[3];
	...
	switch (trdinmode) {
	case DE_3D_SRC_MODE_TB:
		actual_size[0].width = size[0].width;
		actual_size[0].height = size[0].height >> 1;
		actual_size[1].width = size[1].width;
		actual_size[1].height = size[1].height;
		actual_size[2].width = size[2].width;
		actual_size[2].height = size[2].height;
		break;
	case DE_3D_SRC_MODE_SSH:
	case DE_3D_SRC_MODE_SSF:
		actual_size[0].width = size[0].width >> 1;
		actual_size[0].height = size[0].height;
		actual_size[1].width = size[1].width;
		actual_size[1].height = size[1].height;
		actual_size[2].width = size[2].width;
		actual_size[2].height = size[2].height;
	...
	case DE_FORMAT_YUV420_SP_VUVU_10BIT:
	case DE_FORMAT_YUV420_SP_UVUV_10BIT:
		image_w0 = actual_size[0].width;
		image_w2 = image_w1 = actual_size[1].width >> 1;
		image_h0 = actual_size[0].height;
		image_h2 = image_h1 = actual_size[1].height >> 1;
		break;
	case DE_FORMAT_YUV422_P:
	case DE_FORMAT_YUV422_SP_VUVU:
	case DE_FORMAT_YUV422_SP_UVUV:
	...
	case DE_FORMAT_YUV411_SP_UVUV:
		image_w0 = actual_size[0].width;
		image_w2 = image_w1 = actual_size[1].width >> 2;
		image_h0 = actual_size[0].height;
		image_h2 = image_h1 = image_h0;
		break;
	default:
	...
	struct disp_layer_config_data *data, u32 layer_num)
{
	...
	for (; data != data_end; ++data) {
		...
		if (data->config.enable) {
			if (pdata_num >= MAX_LAYER_NUM_PER_CHN) {
				DE_WARN("user layer num overflow on chn%d\n",
					chn);
				break;
			}
			...
			if (data->config.info.fb.metadata_flag
				& SUNXI_METADATA_FLAG_HDRVIVID_HEADER) {
				hdr_type = HDRVIVID;
				pMetadata = data->config.info.fb.p_metadata;
			}
			/* 3d mode*/
			if (data->config.info.fb.flags) {
				/* channel ovl does not support two fbd */
				if (data->config.info.fb.fbd_en)
					continue;
	...
	if (HDR10P == hdr_type || HDRVIVID == hdr_type) {
	...
	de_dlc_set_corlor_range(disp, chn, chn_info->color_range);
	return 0;
}
