...
{
	if (irq_type & DISP_AL_IRQ_TPYE_RCQ)
		return de_top_rcq_enable_irq(disp, irq_flag & DISP_AL_IRQ_FLAG_RCQ_MASK, en);

	if (irq_type & DISP_AL_IRQ_TPYE_RTMX_GLABAL)
		return de_top_enable_irq(disp, irq_flag & DISP_AL_IRQ_FLAG_MASK, en);
...
{
	if (irq_type & DISP_AL_IRQ_TPYE_RCQ)
		return de_top_rcq_query_state_with_clear(disp, irq_state);
...
{
	return de_top_rcq_query_state_is_busy(disp);
}
