...
struct de_top_dump_regs_priv {
	struct dump_regs_module *mod;
};

struct de_top_dump_regs_priv dr_top_priv;
...
{
	writel(((readl(de_base + DE_RTWB_MUX_OFFSET) & 0xFFFFFFF8) | id),
...
#define TOP_MAX_BLOCK_SIZE (0x200)
static int de_top_get_enable_info(const struct dump_regs_user_setting *setting, char *buf)
{
	return 0;
}

static int de_top_get_enable_status(const struct dump_regs_user_setting *setting)
{
	return 0;
}

static int de_top_dump_by_user_setting(const struct dump_regs_user_setting *setting, char *buf)
{
	unsigned int i, num = 0;
	u8 __iomem *addr_base = (u8 __iomem *)de_top_get_reg_base() + 0x8000;
	u32 *value = (u32 *)dr_top_priv.mod->par;
	int addr_offset = 0x8000;

	num += sprintf(buf + num, "%s module:\n\n", dr_top_priv.mod->name);

	de_lowlevel_ioread(addr_base, (void *)value, TOP_MAX_BLOCK_SIZE);
	for (i = 0; i < TOP_MAX_BLOCK_SIZE / 4; i += 4) {
...
int de_top_register_dr_module(void)
{
	struct dump_regs_module *mod = dr_module_alloc(TOP_MAX_BLOCK_SIZE);

	dr_top_priv.mod = mod;

	mod->type = DISP_DE_TOP;
	strcpy(mod->name, module_name[mod->type]);
	mod->ops.dump_by_user_setting = de_top_dump_by_user_setting;
	mod->ops.get_enable_info = de_top_get_enable_info;
	mod->ops.get_enable_status = de_top_get_enable_status;
	register_dr_module(mod);

	return 0;
}

int de_top_unregister_dr_module(void)
{
	unregister_dr_module(dr_top_priv.mod);
	return 0;
}
