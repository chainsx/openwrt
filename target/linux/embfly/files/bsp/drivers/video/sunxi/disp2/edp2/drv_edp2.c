...
s32 edp_misc_parse(struct platform_device *pdev)
{
	...
	ret = of_property_read_string(pdev->dev.of_node, "force_mode", &str);
	if (ret == 0) {
		if (strcmp(str, "dp") == 0)
			g_edp_info[sel].edp_core.controller_mode = 1;
...
static s32 edp_init(struct platform_device *pdev)
{
	...
	g_edp_info[sel].boot_edp = boot_edp;
...
s32 edp_running_thread(void *parg)
{
	...
	while (1) {
		...
		if (!p_edp_info->suspend) {
			...
			if (edp_core_check_controller_error(sel))
				edp_soft_reset(sel);
...
s32 edp_output_sw_enable_early(u32 sel)
{
	...
	g_edp_info[sel].sw_enabled = true;
...
{
	/* if edp is not enable or smooth enable not finish, skip it to
	 * avoid disable the resource has not enabled yet
	 */
	if (g_edp_info[sel].boot_edp) {
		if (g_edp_info[sel].sw_enabled && g_edp_info[sel].enable) {
			edp_output_disable(sel);
			msleep(1);
			edp_output_enable_early(sel);
			edp_output_enable(sel);
		}
	} else if (g_edp_info[sel].enable) {
		edp_output_disable(sel);
		msleep(1);
		edp_output_enable_early(sel);
		edp_output_enable(sel);
