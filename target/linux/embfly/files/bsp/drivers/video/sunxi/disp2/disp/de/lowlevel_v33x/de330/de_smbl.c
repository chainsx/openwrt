...
struct de_smbl_dump_regs_priv {
	struct dump_regs_module *mod;
};

struct de_smbl_dump_regs_priv dr_smbl_priv;
...
static s32 smbl_init(u32 disp, uintptr_t reg_base)
{
	...
	if (g_smbl_status[disp] == NULL) {
		DE_WARN("malloc g_smbl_status[%d] memory fail! size=0x%x\n", disp,
	...
	if (ret != 1) {
		...
		/* DE_INFO("lcdgamma4iep for lcd%d = %d\n", disp, value); */
		if (value > 30 || value < 10) {
			/* DE_WARN("lcdgamma4iep for lcd%d too small or too
	...
	if (ret != 1) {
		/* DE_INFO("smartbl_low_limit for lcd%d not exist\n", disp); */
	} else {
		/* DE_INFO("smartbl_low_limit for lcd%d = %d\n", disp, value); */
		if (value > 255 || value < 20)
			DE_INFO("smartbl_low_limit of lcd%d must be in<20,255>\n",
			    disp);
		else
...
static u16 *PWRSAVE_CORE(u32 disp)
{
	...
	if (backlight < PWRSAVE_PROC_THRES) {
		...
		if (printf_cnt == 600) {
			DE_INFO("save backlight power: %d percent\n",
...
{
	DE_INFO("disp%d, en=%d, win=<%d,%d,%d,%d>\n", disp, info->enable,
...
static int de_smbl_get_enable_info(const struct dump_regs_user_setting *setting, char *buf)
{

	return 0;
}

static int de_smbl_get_enable_status(const struct dump_regs_user_setting *setting)
{
	int disp = setting->index.disp;
	struct de_smbl_private *priv = &(smbl_priv[disp]);
	struct smbl_reg __iomem *regs = (struct smbl_reg *)(priv->reg_blks[0].reg_addr);
	u32 value = 0;
	u32 enable;
	...
	de_lowlevel_ioread(&(regs->gnectl), (void *)&value, 4);
	enable = ((union __imgehc_gnectl_reg_t)value).bits.en & 0x1;
	DUMP_REGS_PRINT("smbl: disp%d enable%d\n", disp, enable);

	return enable;
}

static int de_smbl_dump_by_user_setting(const struct dump_regs_user_setting *setting, char *buf)
{
	unsigned int num = 0;
	struct de_smbl_private *priv;
	int i, disp, screen_num, is_enable, is_support;

	num += sprintf(buf + num, "%s module:\n\n", dr_smbl_priv.mod->name);

	if (setting->use_index_find) {
		disp = setting->index.disp;
		priv = &(smbl_priv[disp]);
		is_enable = de_smbl_get_enable_status(setting);
		is_support = de_feat_is_support_smbl(disp);

		num += sprintf(buf + num, "index(%d) (%s) (%s)\n", disp, is_support ? "supported" : "not supported", is_enable ? "enabled" : "disabled");
		if ((is_enable || setting->force_dump) && is_support)
			num += do_dump_regs_block(dr_smbl_priv.mod, priv->reg_blks, SMBL_CSC_REG_BLK + 1, buf + num);
		num += sprintf(buf + num, "\n");
	} else {
		...
		for (i = 0; i < screen_num; i++) {
			...
			priv = &(smbl_priv[i]);
			is_enable = de_smbl_get_enable_status(&msetting);
			is_support = de_feat_is_support_smbl(i);

			num += sprintf(buf + num, "index(%d) (%s) (%s)\n", i, is_support ? "supported" : "not supported", is_enable ? "enabled" : "disabled");
			if ((is_enable || setting->force_dump) && is_support)
				num += do_dump_regs_block(dr_smbl_priv.mod, priv->reg_blks, SMBL_CSC_REG_BLK + 1, buf + num);
			num += sprintf(buf + num, "\n");
		}
...
int de_smbl_register_dr_module(void)
{
#define SMBL_MAX_BLOCK_SIZE (0x40)
	struct dump_regs_module *mod = dr_module_alloc(SMBL_MAX_BLOCK_SIZE);;

	dr_smbl_priv.mod = mod;

	mod->type = DISP_DRC_MODULE;
	strcpy(mod->name, module_name[mod->type]);
	mod->ops.dump_by_user_setting = de_smbl_dump_by_user_setting;
	mod->ops.get_enable_info = de_smbl_get_enable_info;
	mod->ops.get_enable_status = de_smbl_get_enable_status;
	register_dr_module(mod);

	return 0;
}

int de_smbl_unregister_dr_module(void)
{
	unregister_dr_module(dr_smbl_priv.mod);
	return 0;
}
