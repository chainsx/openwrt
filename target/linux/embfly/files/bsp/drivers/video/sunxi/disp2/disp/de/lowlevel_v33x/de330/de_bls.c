...
struct de_bls_dump_regs_priv {
	struct dump_regs_module *mod;
};

struct de_bls_dump_regs_priv dr_bls_priv;
...
static int de_bls_get_enable_info(const struct dump_regs_user_setting *setting, char *buf)
{

	return 0;
}

static int de_bls_get_enable_status(const struct dump_regs_user_setting *setting)
{
	int disp = setting->index.disp;
	int chn = setting->index.chn;
	struct de_bls_private *priv = &(bls_priv[disp][chn]);
	struct bls_reg __iomem *regs = (struct bls_reg *)(priv->reg_blks[0].reg_addr);
	u32 value = 0;
	u32 enable;
	...
	enable = ((union bls_ctl_reg)value).bits.en & 0x1;
	DUMP_REGS_PRINT("bls: disp%d chn%d enable%d\n", disp, chn, enable);

	return enable;
}

static int de_bls_dump_by_user_setting(const struct dump_regs_user_setting *setting, char *buf)
{
	unsigned int num = 0;
	struct de_bls_private *priv;
	int i, j, disp, chn, screen_num, chn_num, is_enable, is_support;

	num += sprintf(buf + num, "%s module:\n\n", dr_bls_priv.mod->name);

	if (setting->use_index_find) {
		disp = setting->index.disp;
		chn = setting->index.chn;
		priv = &(bls_priv[disp][chn]);
		is_enable = de_bls_get_enable_status(setting);
		...
		if ((is_enable || setting->force_dump) && is_support)
			num += do_dump_regs_block(dr_bls_priv.mod, priv->reg_blks, ARRAY_SIZE(priv->reg_blks), buf + num);
		num += sprintf(buf + num, "\n");
	} else {
		...
		for (i = 0; i < screen_num; i++) {
			...
			for (j = 0; j < chn_num; j++) {
				...
				priv = &(bls_priv[i][j]);
				is_enable = de_bls_get_enable_status(&msetting);
...
int de_bls_register_dr_module(void)
{
#define BLS_MAX_BLOCK_SIZE (0x40)
	struct dump_regs_module *mod = dr_module_alloc(BLS_MAX_BLOCK_SIZE);

	dr_bls_priv.mod = mod;

	mod->type = DISP_BLS_MODULE;
	strcpy(mod->name, module_name[mod->type]);
	mod->ops.dump_by_user_setting = de_bls_dump_by_user_setting;
	mod->ops.get_enable_info = de_bls_get_enable_info;
	mod->ops.get_enable_status = de_bls_get_enable_status;
	register_dr_module(mod);

	return 0;
}

int de_bls_unregister_dr_module(void)
{
	unregister_dr_module(dr_bls_priv.mod);
	return 0;
}
