...
int _parse_data_block_detailed_timing(dw_hdmi_dev_t *dev, dw_dtd_t *dtd, u8 data[18])
{
	...
	dtd->mPixelClock = 10 * byte_to_word(data[1], data[0]);/* [10000Hz] */
...
static int _edid_parse_standard_timing(struct edid *edid, sink_edid_t *sink)
{
	u32 hactive = 0, vactive = 0, rate = 0;
	u32 i = 0, j = 0, ratio = 0;
	u8 ratio_arry[4][2] = {{16, 10}, {4, 3}, {5, 4}, {16, 9}};
	...
	for (i = 0; i < 8; i++) {
		...
		hactive = edid->standard_timings[i].hsize * 8 + 248;
		ratio = (edid->standard_timings[i].vfreq_aspect & 0xC0) >> 6;
		vactive = (hactive / ratio_arry[ratio][0]) * ratio_arry[ratio][1];
		rate = (edid->standard_timings[i].vfreq_aspect & 0x3F) + 60;

		/* match in dtd table and save vic */
		for (j = 0; _dtd[j].dtd.mCode != 0; j++) {
			if (vactive != _dtd[j].dtd.mVActive)
				continue;
			if (hactive != _dtd[j].dtd.mHActive)
				continue;
			if ((rate * 1000) != _dtd[j].refresh_rate)
				continue;
			if (ratio_arry[ratio][0] != _dtd[j].dtd.mHImageSize)
				continue;
			if (ratio_arry[ratio][1] != _dtd[j].dtd.mVImageSize)
				continue;

			sink->edid_mStandard[sink->edid_mStandardIndex].code = _dtd[j].dtd.mCode;
			sink->edid_mStandard[sink->edid_mStandardIndex].hactive = hactive;
			sink->edid_mStandard[sink->edid_mStandardIndex].vactive = vactive;
			sink->edid_mStandard[sink->edid_mStandardIndex].rate   = rate;
			sink->edid_mStandardIndex++;
			hdmi_inf("sink save standard timing vic: 0x%x %dx%d@%dHz %d:%d\n",
				_dtd[j].dtd.mCode, hactive, vactive, rate,
				ratio_arry[ratio][0], ratio_arry[ratio][1]);
			break;
		}
	}
	return 0;
...
static int
_edid_parse_dtd_block(dw_hdmi_dev_t *dev, sink_edid_t *sink, dw_dtd_t dtd)
{
	/* check sink edid dtd buffer size */
	if (sink->edid_mDtdIndex > (AW_EDID_DTD_MAX_SIZE - 1)) {
		hdmi_wrn("sink edid dtd buffer is full\n");
		return 0;
	}

	sink->edid_mDtd[sink->edid_mDtdIndex] = dtd;
	EDID_INF("detailed timing[%d] pixel %dHz %dx%d\n",
		sink->edid_mDtdIndex,
		sink->edid_mDtd[sink->edid_mDtdIndex].mPixelClock,
		sink->edid_mDtd[sink->edid_mDtdIndex].mHActive,
		sink->edid_mDtd[sink->edid_mDtdIndex].mVActive);
		...
		struct edid *edid, sink_edid_t *sink)
{
	...
	EDID_INF("Block0 parse standard timing\n");
	_edid_parse_standard_timing(edid, sink);

	EDID_INF("Block0 parse detailed discriptor\n");
	for (i = 0; i < 4; i++) {
		...
		if (detailed_timing->pixel_clock == 0) {
			...
			/* parse and save block0 dtd timing */
			ret = _parse_data_block_detailed_timing(dev, &tmpDtd, (u8 *)detailed_timing);
			if (ret != true)
				continue;

			_edid_parse_dtd_block(dev, sink, tmpDtd);
		}
	}
	return true;
}
		...
		u8 *buffer, sink_edid_t *sink)
{
	...
	sink->edid_mYcc422Support     = bit_field(buffer[3],	4, 1) == 1;
	sink->edid_mYcc444Support     = bit_field(buffer[3],	5, 1) == 1;
	sink->edid_mBasicAudioSupport = bit_field(buffer[3], 6, 1) == 1;
	sink->edid_mUnderscanSupport  = bit_field(buffer[3], 7, 1) == 1;

	/* CTA 861 data block parse */
	if (offset != 4) {
		for (i = 4; i < offset; i += _parse_cta_data_block(dev, buffer + i, sink))
			;
	}

	memcpy(sink->detailed_timings, buffer + offset, sizeof(sink->detailed_timings));

	/* last is checksum */
	for (i = offset, c = 0; ((i + DTD_SIZE) < (EDID_BLOCK_SIZE - 1)) && c < 6; i += DTD_SIZE, c++) {
		ret = _parse_data_block_detailed_timing(dev, &tmpDtd, buffer + i);
		if (ret != true)
			continue;
...
void dw_edid_reset(dw_hdmi_dev_t *dev, sink_edid_t *edidExt)
{
	...
	memset(edidExt->edid_mDtd, 0x0, sizeof(dw_dtd_t *) * AW_EDID_DTD_MAX_SIZE);
