...
void dw_phy_interrupt_all_mask(dw_hdmi_dev_t *dev)
{
	_dw_phy_interrupt_mask(dev, 0xff);
}

u32 dw_phy_get_ir_state(dw_hdmi_dev_t *dev)
{
	return dev_read(dev, IH_PHY_STAT0);
}

void dw_phy_clear_ir_state(dw_hdmi_dev_t *dev, u32 state)
{
	dev_write(dev, IH_PHY_STAT0, state);
}
