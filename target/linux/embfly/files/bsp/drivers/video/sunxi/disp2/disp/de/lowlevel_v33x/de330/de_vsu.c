...
struct de_vsu_dump_regs_priv {
	struct dump_regs_module *mod;
};

struct de_vsu_dump_regs_priv dr_vsu_priv;
	...
	struct de_scale_para *cpara)
{
	...
	if (fm_space == DE_FORMAT_SPACE_RGB) {
		...
		DE_WARN("calc cpara for fm_space(%d)\n", fm_space);
		return -1;
	}
	...
	if (chn_info->yuv_sampling == DE_YUV422) {
		...
		DE_WARN("not support yuv_sampling(%d)\n", chn_info->yuv_sampling);
		return -1;
	}
	...
	u32 dst_coord, u32 scale_step)
{
	...
	DE_INFO("half_shift_coord=<%x,%d,%d,%d>\n", half,
	...
	struct de_chn_info *chn_info)
{
	...
	if (chn_info->px_fmt_space == DE_FORMAT_SPACE_RGB) {
		...
		pt_coef = de_vsu_calc_fir_coef(chn_info->ovl_cpara.hstep);
		aw_memcpy_toio(reg->c_hori_coeff0,
			bicubic8coefftab32_left + pt_coef,
			sizeof(u32) * VSU_PHASE_NUM);
		aw_memcpy_toio(reg->c_hori_coeff1,
			bicubic8coefftab32_right + pt_coef,
			sizeof(u32) * VSU_PHASE_NUM);
		pt_coef = de_vsu_calc_fir_coef(chn_info->ovl_cpara.vstep);
		aw_memcpy_toio(reg->c_vert_coeff,
	...
	for (chn = 0; chn < chn_num; ++chn) {
		...
		case DE_SCALER_TYPE_VSU8:
			...
			if (NULL == reg_mem_info->vir_addr) {
				DE_WARN("alloc vsu[%d][%d] mm fail!size=0x%x\n",
		...
		case DE_SCALER_TYPE_VSU_ED:
			...
			break;
		default:
			DE_WARN("not support this vsu_type=%d\n",
				priv->vsu_type);
			break;
		}
...
static void __iomem *get_scaler_io_regs(struct de_vsu_private *priv)
{
	switch (priv->vsu_type) {
	case DE_SCALER_TYPE_GSU:
		return (void *)(priv->reg_blks[0].reg_addr);
		break;
	case DE_SCALER_TYPE_VSU8:
		return (void *)(priv->vsu8_reg_blks[0].reg_addr);
		break;
	case DE_SCALER_TYPE_VSU10:
		return (void *)(priv->vsu10_reg_blks[0].reg_addr);
		break;
	case DE_SCALER_TYPE_VSU_ED:
		return (void *)(priv->vsu_ed_reg_blks[0].reg_addr);
		break;
	default:
		break;
	}
...
static void get_scaler_block_info(struct de_vsu_private *priv, struct de_reg_block **list, unsigned int *num)
{
	switch (priv->vsu_type) {
	case DE_SCALER_TYPE_GSU:
		break;
	case DE_SCALER_TYPE_VSU8:
		*list = priv->vsu8_reg_blks;
		*num = VSU8_REG_BLK_NUM;
		break;
	case DE_SCALER_TYPE_VSU10:
		*list = priv->vsu10_reg_blks;
		*num = VSU10_REG_BLK_NUM;
		break;
	case DE_SCALER_TYPE_VSU_ED:
		*list = priv->vsu_ed_reg_blks;
		*num = VSU_ED_REG_BLK_NUM;
		break;
	default:
		break;
	}
...
static int de_vsu_get_enable_info(const struct dump_regs_user_setting *setting, char *buf)
{

	return 0;
}

static int de_vsu_get_enable_status(const struct dump_regs_user_setting *setting)
{
	int disp = setting->index.disp;
	int chn = setting->index.chn;
	struct de_vsu_private *priv = &(vsu_priv[disp][chn]);
	void __iomem *regs = get_scaler_io_regs(priv);
	struct vsu8_reg *v8reg;
	struct vsu10_reg *v10reg;
	struct vsu_ed_reg *vedreg;
	u32 value = 0;
	u32 enable;
	...
	case DE_SCALER_TYPE_GSU:
		...
		break;
	case DE_SCALER_TYPE_VSU8:
		v8reg = (struct vsu8_reg *)regs;
		de_lowlevel_ioread(&(v8reg->ctl), (void *)&value, 4);
		enable = ((union vsu8_ctl_reg)value).bits.en & 0x1;
		break;
	case DE_SCALER_TYPE_VSU10:
		v10reg = (struct vsu10_reg *)regs;
		de_lowlevel_ioread(&(v10reg->ctl), (void *)&value, 4);
		enable = ((union vsu10_ctl_reg)value).bits.en & 0x1;
		break;
	case DE_SCALER_TYPE_VSU_ED:
		vedreg = (struct vsu_ed_reg *)regs;
		de_lowlevel_ioread(&(vedreg->ctl), (void *)&value, 4);
		enable = ((union vsu_ed_ctl_reg)value).bits.en & 0x1;
		break;
	default:
		enable = 0;
		break;
	...
	DUMP_REGS_PRINT("scaler: disp%d chn%d enable%d\n", disp, chn, enable);

	return enable;
}

static int de_vsu_dump_by_user_setting(const struct dump_regs_user_setting *setting, char *buf)
{
	unsigned int num = 0;
	struct de_vsu_private *priv;
	int i, j, disp, chn, screen_num, chn_num, is_enable, block_size;
	struct de_reg_block *block_list;

	num += sprintf(buf + num, "%s module:\n\n", dr_vsu_priv.mod->name);

	if (setting->use_index_find) {
		disp = setting->index.disp;
		chn = setting->index.chn;
		priv = &(vsu_priv[disp][chn]);
		is_enable = de_vsu_get_enable_status(setting);
		get_scaler_block_info(priv, &block_list, &block_size);

		DUMP_REGS_PRINT("scaler: list %lx, size%d, type %d", (uintptr_t)block_list, block_size, priv->vsu_type);

		num += sprintf(buf + num, "index(%d, %d) (%s)\n", disp, chn, is_enable ? "enabled" : "disabled");
		if (is_enable || setting->force_dump)
			num += do_dump_regs_block(dr_vsu_priv.mod, block_list, block_size, buf + num);
		num += sprintf(buf + num, "\n");
	} else {
		...
		for (i = 0; i < screen_num; i++) {
			...
			for (j = 0; j < chn_num; j++) {
				...
				priv = &(vsu_priv[i][j]);
				is_enable = de_vsu_get_enable_status(&msetting);
				get_scaler_block_info(priv, &block_list, &block_size);

				DUMP_REGS_PRINT("scaler: list %lx, size%d, type %d", (uintptr_t)block_list, block_size, priv->vsu_type);

				num += sprintf(buf + num, "index(%d, %d) (%s)\n", i, j, is_enable ? "enabled" : "disabled");
				if (is_enable || setting->force_dump)
					num += do_dump_regs_block(dr_vsu_priv.mod, block_list, block_size, buf + num);
				num += sprintf(buf + num, "\n");
			}
...
int de_vsu_register_dr_module(void)
{
#define SCALER_MAX_BLOCK_SIZE (128)
	struct dump_regs_module *mod = dr_module_alloc(SCALER_MAX_BLOCK_SIZE);

	dr_vsu_priv.mod = mod;

	mod->type = DISP_SCALER_MODULE;
	strcpy(mod->name, module_name[mod->type]);
	mod->ops.dump_by_user_setting = de_vsu_dump_by_user_setting;
	mod->ops.get_enable_info = de_vsu_get_enable_info;
	mod->ops.get_enable_status = de_vsu_get_enable_status;
	register_dr_module(mod);

	return 0;
}

int de_vsu_unregister_dr_module(void)
{
	unregister_dr_module(dr_vsu_priv.mod);
	return 0;
}
