...
s32 de_feat_is_support_dlc_by_chn(u32 disp, u32 chn)
{
	if ((disp < de_feat_get_num_screens())
		&& (chn < de_feat_get_num_chns(disp))) {
		u32 i, index = 0;
		for (i = 0; i < disp; i++)
			index += de_feat_get_num_chns(i);
		index += chn;
		return de_cur_features->is_support_dlc[index];
	}
	return 0;
}

s32 de_feat_is_support_gamma_by_chn(u32 disp, u32 chn)
{
	if ((disp < de_feat_get_num_screens())
		&& (chn < de_feat_get_num_chns(disp))) {
		u32 i, index = 0;
		for (i = 0; i < disp; i++)
			index += de_feat_get_num_chns(i);
		index += chn;
		return de_cur_features->is_support_chn_gamma[index];
	}
	return 0;
}
