...
struct disp_lcd_private_data {
	...
	struct reset_control *rst_bus_tcon_lcd_bus;
#endif
...
static s32 lcd_parse_panel_para(u32 disp, struct disp_panel_para *info)
{
	...
	ret = disp_sys_script_get_item(primary_key, "lcd_hs_trail_set", &value, 1);
	if (ret == 1)
...
#if defined(HAVE_TCON_BUS_RESET_CLK)
	ret = reset_control_deassert(lcdp->rst_bus_tcon_lcd_bus);
	if (ret) {
		DE_WARN("reset_control_deassert for rst_bus_tcon_lcd_bus failed\n");
		return ret;
	}
#endif
...
#if defined(HAVE_TCON_BUS_RESET_CLK)
	ret = reset_control_assert(lcdp->rst_bus_tcon_lcd_bus);
	if (ret) {
		DE_WARN("reset_control_assert for rst_bus_tcon_lcd_bus failed\n");
		return ret;
	}
#endif
...
#if defined(HAVE_TCON_BUS_RESET_CLK)
		lcdp->rst_bus_tcon_lcd_bus = para->rst_bus_tcon_bus[lcd->hwdev_index];
#endif
