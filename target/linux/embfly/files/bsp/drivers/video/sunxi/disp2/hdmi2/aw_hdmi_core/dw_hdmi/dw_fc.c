...
void dw_fc_video_force_value(dw_hdmi_dev_t *dev, u8 enable, u32 value)
{
	if (enable) {
		/* enable force frame composer video output */
		dev_write(dev, FC_DBGTMDS2, (u8)((value >> 16) & 0xFF)); /* R */
		dev_write(dev, FC_DBGTMDS1, (u8)((value >> 8)  & 0xFF)); /* G */
		dev_write(dev, FC_DBGTMDS0, (u8)((value >> 0)  & 0xFF)); /* B */
	}
	dev_write_mask(dev, FC_DBGFORCE, FC_DBGFORCE_FORCEVIDEO_MASK, enable ? 0x1 : 0x0);
}
...
int dw_fc_vsif_config(dw_hdmi_dev_t *dev, u8 enable)
{
	dev_write_mask(dev, FC_DATAUTO1,
			FC_DATAUTO1_AUTO_FRAME_INTERPOLATION_MASK, enable ? 0x1 : 0x0);
	dev_write_mask(dev, FC_DATAUTO2,
			FC_DATAUTO2_AUTO_FRAME_PACKETS_MASK, enable ? 0x1 : 0x0);
	dev_write_mask(dev, FC_DATAUTO2,
			FC_DATAUTO2_AUTO_LINE_SPACING_MASK, 0x0);
	_dw_vsi_enable(dev, enable ? 0x1 : 0x0);
