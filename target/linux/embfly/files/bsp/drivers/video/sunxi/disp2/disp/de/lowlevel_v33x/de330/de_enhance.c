			   ...
			   struct disp_enhance_config *config)
{
	...
	u32 saturation;
	u32 hue;
	u32 edge, detail, denoise;

	chno = g_cfg->vep_num[disp];

	/* the new enhance is 0-100, mapping 0-100 to 0-10 */
	bright = (config[0].info.bright + 9) / 10;
	contrast = (config[0].info.contrast + 9) / 10;
	saturation = (config[0].info.saturation + 9) / 10;
	hue = (config[0].info.hue + 9) / 10;
	edge = (config[0].info.edge + 9) / 10;
	detail = (config[0].info.detail + 9) / 10;
	denoise = (config[0].info.denoise + 9) / 10;
	DE_INFO("bright %d contrast %d saturation %d hue %d edge %d detail %d denoise %d",
		 bright, contrast, saturation, hue, edge, detail, denoise);
	...
	for (chn = 0; chn < chno; chn++) {
		...
		if (config[0].flags & ENH_BRIGHT_DIRTY) {
			g_para[disp][chn]->fce_para.bright_level = bright;
		}

		if (config[0].flags & ENH_CONTRAST_DIRTY) {
			g_para[disp][chn]->fce_para.contrast_level = contrast;
		}

		if (config[0].flags & ENH_EDGE_DIRTY) {
			g_para[disp][chn]->lti_para.level = edge;
		}

		if (config[0].flags & ENH_DETAIL_DIRTY) {
			g_para[disp][chn]->peak_para.level = detail;
			g_para[disp][chn]->peak2d_para.level = detail;
		}

		if (config[0].flags & ENH_SAT_DIRTY) {
			g_para[disp][chn]->fcc_para.level = saturation;
			g_para[disp][chn]->bls_para.level = saturation;
		}

		if (config[0].flags & ENH_DNS_DIRTY) {
			g_para[disp][chn]->dns_para.level = denoise;
		}
	}
	...
	for (disp = 0; disp < g_cfg->device_num; disp++)
		for (chn = 0; chn < g_cfg->vep_num[disp]; chn++) {
			...
			if (g_alg[disp][chn] == NULL) {
				DE_WARN("malloc g_alg[%d][%d] memory fail!",
				      disp, chn);
				DE_WARN("size=0x%x\n",
