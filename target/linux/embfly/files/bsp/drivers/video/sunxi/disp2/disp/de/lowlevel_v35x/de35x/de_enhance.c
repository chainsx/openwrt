...
static s32 de_enhance_init_para(u32 disp, u32 chn)
{
	...
	if (g_cfg->dlc_exist[disp][chn])
		de_dlc_init_para(disp, chn);
	...
	if (chn == 0) {
		if (de_feat_is_support_gamma(disp))
			de_gamma_init_para(disp, 0, DISP_GAMMA_TYPE);
...
#define DEMO_SPEED 200
	...
	/* dlc */
	if (g_cfg->dlc_exist[disp][chn]) {
		de_dlc_set_size(disp, chn, size.width, size.height);
		/* de_dlc_set_window(disp, chn, demo_enable, tmp_win); */
	}
	...
	if (chn == 0) {
		/* gamma */
		if (de_feat_is_support_gamma(disp)) {
			de_gamma_set_size(disp, 0, DISP_GAMMA_TYPE, size3.width, size3.height);
...
s32 de_enhance_layer_apply(u32 disp, struct disp_enhance_chn_info *info)
{
	...
	for (chn = 0; chn < chno; chn++) {
		...
		if (flags & ENH_FORMAT_DIRTY) {
			...
			/* convert rgb to yuv using sharp's csc */
			g_para[disp][chn]->fmt = format = 0;
#else
			...
			if (g_cfg->dlc_exist[disp][chn])
				de_dlc_enable(disp, chn, format == 0 ? 1 : 0);
			if (g_cfg->sharp_exist[disp][chn])
...
s32 de_enhance_rcq_finish_tasklet(u32 disp)
{
	...
	for (chn = 0; chn < chno; chn++) {
		...
		/* dlc */
		if (g_cfg->dlc_exist[disp][chn])
			de_dlc_tasklet(disp, chn, g_alg[disp][chn]->frame_cnt,
...
s32 de_enhance_init(struct disp_bsp_init_para *para)
{
	...
	for (disp = 0; disp < g_cfg->device_num; disp++) {
		for (chn = 0; chn < chn_num[disp]; chn++) {
			...
			g_cfg->dlc_exist[disp][chn] =
				de_feat_is_support_dlc_by_chn(disp, chn);
			      ...
			      u32 *blk_num)
{
	...
	if (blks == NULL) {
		for (chn = 0; chn < chn_num; ++chn) {
			if (de_feat_is_support_vep_by_chn(disp, chn)) {
				...
				if (g_cfg->dlc_exist[disp][chn]) {
					de_dlc_get_reg_blocks(disp, chn, blks,
							      &num);
					total += num;
				}
				if (g_cfg->fcm_exist[disp][chn]) {
	...
	for (chn = 0; chn < chn_num; ++chn) {
		if (de_feat_is_support_vep_by_chn(disp, chn)) {
			...
			if (g_cfg->dlc_exist[disp][chn]) {
				num = *blk_num;
				de_dlc_get_reg_blocks(disp, chn, blks, &num);
				total += num;
				blks += num;
				*blk_num -= num;
			}
