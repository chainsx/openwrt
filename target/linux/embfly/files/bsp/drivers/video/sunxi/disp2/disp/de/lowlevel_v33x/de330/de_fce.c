...
struct de_fce_dump_regs_priv {
	struct dump_regs_module *mod;
};

struct de_fce_dump_regs_priv dr_fce_priv;
	...
	u8 __iomem **phy_addr, u8 **vir_addr, u32 *size)
{
	...
	*size -= DE_FCE_REG_MEM_SIZE;
	...
	if (g_hist[disp][chn] == NULL) {
		DE_WARN("malloc hist[%d][%d] memory fail! size=0x%x\n",
		disp, chn, 1024);
		return -1;
	}
	g_hist_p[disp][chn] = kmalloc(1024, GFP_KERNEL | __GFP_ZERO);
	if (g_hist_p[disp][chn] == NULL) {
		DE_WARN("malloc g_hist_p[%d][%d] memory fail! size=0x%x\n", disp,
		      chn, 1024);
		return -1;
	}
	/* ce */
	...
	if (g_ce_status[disp][chn] == NULL) {
		DE_WARN("malloc g_ce_status[%d][%d] memory fail! size=0x%x\n",
	...
	if (g_celut[disp][chn] == NULL) {
		DE_WARN("malloc celut[%d][%d] memory fail! size=0x%x\n",
		      disp, chn, 512);
		return -1;
	}
	...
	if (hist_res[disp][chn] == NULL) {
		DE_WARN("malloc hist_res[%d][%d] memory fail! size=0x%x\n", disp,
			   ...
			   u32 hist[256], u32 *sum)
{
	...
	aw_memcpy_fromio(hist, hw_reg->hist, sizeof(u32) * 256);
		  ...
		  u16 ce_lut[256])
{
	...
	aw_memcpy_toio(reg->celut, (u8 *)ce_lut, sizeof(u16) * 256);
		     ...
		     struct __fce_config_data *para, u32 bypass)
{
	...
	if (b_level >= FCE_PARA_NUM || c_level >= FCE_PARA_NUM || dev_type >= 2) {
		DE_WARN("fce para over range b_level %d c_level %d dev_type %d",
			 b_level, c_level, dev_type);
		return -1;
	}
	...
	if ((mean == 0) || (p_hist_data->hist_mean < ce_thr)
	  || total_size == 0) {
		...
		if (p_hist_data->white_thr0 >= 256) {
			DE_WARN("p_hist_data->white_thr0(%d) >= 256",
...
static int de_fce_get_enable_info(const struct dump_regs_user_setting *setting, char *buf)
{

	return 0;
}

static int de_fce_get_enable_status(const struct dump_regs_user_setting *setting)
{
	int disp = setting->index.disp;
	int chn = setting->index.chn;
	struct de_fce_private *priv = &(fce_priv[disp][chn]);
	struct fce_reg __iomem *regs = (struct fce_reg *)(priv->reg_blks[0].reg_addr);
	u32 value = 0;
	u32 enable;
	...
	enable = ((union fce_gctrl_reg)value).bits.en & 0x1;
	DUMP_REGS_PRINT("fce: disp%d chn%d enable%d\n", disp, chn, enable);

	return enable;
}

static int de_fce_dump_by_user_setting(const struct dump_regs_user_setting *setting, char *buf)
{
	unsigned int num = 0;
	struct de_fce_private *priv;
	int i, j, disp, chn, screen_num, chn_num, is_enable, is_support;

	num += sprintf(buf + num, "%s module:\n\n", dr_fce_priv.mod->name);

	if (setting->use_index_find) {
		disp = setting->index.disp;
		chn = setting->index.chn;
		priv = &(fce_priv[disp][chn]);
		is_enable = de_fce_get_enable_status(setting);
		...
		if ((is_enable || setting->force_dump) && is_support)
			num += do_dump_regs_block(dr_fce_priv.mod, priv->reg_blks, FCE_CSC_REG_BLK + 1, buf + num);
		num += sprintf(buf + num, "\n");
	} else {
		...
		for (i = 0; i < screen_num; i++) {
			...
			for (j = 0; j < chn_num; j++) {
				...
				priv = &(fce_priv[i][j]);
				is_enable = de_fce_get_enable_status(&msetting);
...
int de_fce_register_dr_module(void)
{
#define FCE_MAX_BLOCK_SIZE (0x40)
	struct dump_regs_module *mod = dr_module_alloc(FCE_MAX_BLOCK_SIZE);

	dr_fce_priv.mod = mod;

	mod->type = DISP_FCE_MODULE;
	strcpy(mod->name, module_name[mod->type]);
	mod->ops.dump_by_user_setting = de_fce_dump_by_user_setting;
	mod->ops.get_enable_info = de_fce_get_enable_info;
	mod->ops.get_enable_status = de_fce_get_enable_status;
	register_dr_module(mod);

	return 0;
}

int de_fce_unregister_dr_module(void)
{
	unregister_dr_module(dr_fce_priv.mod);
	return 0;
}
