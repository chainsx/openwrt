...
struct disp_device *disp_get_hdmi(u32 disp)
{
	...
	if (disp >= num_screens
		...
		DE_WARN("disp %d not support HDMI output\n", disp);
		return NULL;
	}
...
static s32 hdmi_clk_init(struct disp_device *hdmi)
{
	...
	if (!hdmi || !hdmip) {
		DE_WARN("hdmi clk init null hdl\n");
		return DIS_FAIL;
	}
	...
	if (hdmip->clk) {
		...
		if (IS_ERR_OR_NULL(hdmip->parent_clk))
			DE_WARN("get hdmi clk parent fail\n");
	}
...
static s32 hdmi_clk_config(struct disp_device *hdmi)
{
	...
	if (!parent_clk) {
		DE_WARN("can not get hdmi prent clk\n");
		return DIS_FAIL;
	}
...
#if IS_ENABLED(CONFIG_ARCH_SUN50IW9)
	/* Warning: If possible, do not modify the parent clock. */
...
static s32 hdmi_clk_enable(struct disp_device *hdmi)
{
	...
	if (hdmip->clk_enabled) {
		...
		DE_WARN("hdmi%d clk already enable!\n", hdmi->disp);
		return 0;
	}
	...
	if (hdmip->bus_clk) {
		...
		if (ret != 0)
			DE_WARN("fail enable hdmi's bus clock\n");
	}
	...
	if (hdmip->clk) {
		...
		if (ret != 0)
			DE_WARN("fail enable hdmi's clock\n");
	}
...
static s32 hdmi_clk_disable(struct disp_device *hdmi)
{
	...
	if (!hdmip->clk_enabled) {
		DE_WARN("hdmi%d clk already disable!\n", hdmi->disp);
		return 0;
	}
...
s32 disp_hdmi_clk_disable(void)
{
	return hdmi_clk_disable(hdmis);
}
EXPORT_SYMBOL(disp_hdmi_clk_disable);
...
#ifdef USE_CEC_DDC_PAD
#if IS_ENABLED(CONFIG_ARCH_SUN50IW9) || IS_ENABLED(CONFIG_ARCH_SUN8IW20)
			      ...
			      struct disp_device_func *func)
{
	...
	if ((hdmi == NULL) || (hdmip == NULL)) {
		DE_WARN("hdmi set func null  hdl\n");
		return DIS_FAIL;
	}
...
static s32 cal_real_frame_period(struct disp_device *hdmi)
{
	...
	if (!hdmip) {
		DE_WARN("hdmip is null\n");
		goto OUT;
	}

	if (!hdmip->clk || !hdmip->video_info) {
		DE_WARN("hdmi clk | video_info null\n");
		goto OUT;
	}
	...
	if (!hdmi->timings.dclk_rate_set) {
		DE_WARN("hdmi dclk_rate_set is 0\n");
		goto OUT;
	}
...
s32 disp_hdmi_enable(struct disp_device *hdmi)
{
	...
	mgr = hdmi->manager;
	if (!mgr) {
		DE_WARN("hdmi%d's mgr is NULL\n", hdmi->disp);
		mutex_unlock(&hdmi_mlock);
		return DIS_FAIL;
	}

	if (hdmip->hdmi_func.get_video_timing_info == NULL) {
		DE_WARN("hdmi_get_video_timing_info func is null\n");
		mutex_unlock(&hdmi_mlock);
		return DIS_FAIL;
	}
	...
	if (hdmip->enabled == 1) {
		DE_WARN("hdmi%d is already enable\n", hdmi->disp);
		goto exit;
	}
...
static s32 disp_hdmi_disable(struct disp_device *hdmi)
{
	...
	if (hdmip->enabled == 0) {
		DE_WARN("hdmi%d is already disable\n", hdmi->disp);
		return DIS_FAIL;
	}
					...
					struct disp_device_config *config)
{
	...
	if (config->type != hdmi->type) {
		DE_WARN("something error! type(0x%x) is error\n", config->type);
		ret = DISP_NOT_UPDATE;
		goto exit;
	}
...
s32 disp_init_hdmi(struct disp_bsp_init_para *para)
{
	...
	if (!node) {
		DE_WARN("do not get the node of hdmi\n");
		goto exit;
	}

	ret = of_property_read_string(node, "status", &str);
	if (ret || strcmp(str, "okay")) {
		DE_WARN("dont support hdmi\n");
		goto exit;
	}
	hdmi_used = 1;

	DE_INFO("disp_init_hdmi\n");
