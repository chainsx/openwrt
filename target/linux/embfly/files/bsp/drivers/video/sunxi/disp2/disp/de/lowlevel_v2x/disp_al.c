...
#if IS_ENABLED(CONFIG_ARCH_SUN8IW12)
...
int disp_al_manager_apply(unsigned int disp, struct disp_manager_data *data)
{
	...
	DE_INFO("ouput_type:%d\n", al_priv.output_type[al_priv.tcon_id[disp]]);
...
int disp_al_capture_init(unsigned int disp, unsigned int port)
{
	int ret = -1;
			     ...
			     struct disp_panel_para *panel)
{
	...
	if (find == 0)
		DE_WARN("cant find clk info for lcd_if %d\n", panel->lcd_if);

#if IS_ENABLED(CONFIG_AW_FPGA_S4) && IS_ENABLED(CONFIG_EINK_PANEL_USED)
...
#endif
		if (de_feat_is_support_gamma(i))
			de_gamma_init(i, para->reg_base[DISP_MOD_DE]);
	}
	...
	for (j = 0; j < DEVICE_NUM; j++) {
		if (para->boot_info[j].sync == 1) {
			...
			if (hwdev_index == DEVICE_NUM) {
				DE_WARN("Can not find tcon index for smooth display\n");
				hwdev_index = 0;
			}
#if defined(HAVE_DEVICE_COMMON_MODULE)
