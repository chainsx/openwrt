...
static u32 get_header_size(u32 width, u32 height, enum disp_pixel_format fmt)
{
#define HEADER_ALIGN (256)

	u32 header_size, width_align, header_align;

	switch (fmt) {
	/* 24/32 bbp block is 16 * 4 */
	case DISP_FORMAT_ARGB_8888:
	case DISP_FORMAT_ABGR_8888:
	case DISP_FORMAT_RGBA_8888:
	case DISP_FORMAT_BGRA_8888:
	...
	case DISP_FORMAT_BGR_888:
	{
		width_align = (width + 15) / 16;
		header_align = (height + 3) / 4;
		header_size = DISPALIGN((width_align * header_align * 8), HEADER_ALIGN);
		break;
	}
	/* 16 bbp block is 32 * 4 */
	case DISP_FORMAT_RGB_565:
	case DISP_FORMAT_BGR_565:
	case DISP_FORMAT_ARGB_1555:
	case DISP_FORMAT_ABGR_1555:
...
{
	u32 y_body_size = 0, width_align, header_align;
	switch (fmt) {
	case DISP_FORMAT_YUV420_SP_UVUV:
	case DISP_FORMAT_YUV420_SP_VUVU:
	{
		width_align = (width + 31) / 16;
		header_align = (height + 7) / 4;
		y_body_size = width_align * header_align * 256;
		break;
	}
	case DISP_FORMAT_YUV420_SP_UVUV_10BIT:
	case DISP_FORMAT_YUV420_SP_VUVU_10BIT:
...
//	tmp = 0x63;
	...
	header_size = get_header_size(width, height, lay_info->fb.format);
	y_addr = addr + header_size;
