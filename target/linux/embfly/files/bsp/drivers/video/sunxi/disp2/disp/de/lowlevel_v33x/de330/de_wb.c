...
struct de_wb_dump_regs_priv {
	struct dump_regs_module *mod;
};

struct de_wb_dump_regs_priv dr_wb_priv;
...
static void de_wb_dump_user_cfg(struct disp_capture_config *cfg)
{
	...
	DE_WARN("disp=%d, flags=%d\n",
		cfg->disp, cfg->flags);

	frame = &cfg->in_frame;
	DE_WARN("in frame: fmt=%d, fd=%d, sz[%d,%d,%d,%d,%d,%d]"
...
static u8 is_valid_para(struct disp_capture_config *cfg)
{
	if ((cfg->in_frame.size[0].width < MININWIDTH)
		...
		/* when the data source exceeds line buf and fine scaler is required, it is not supported */
		|| ((cfg->out_frame.crop.width > LINE_BUF_LEN) &&
		   ((cfg->in_frame.crop.width > (cfg->out_frame.crop.width << 1))
		    && (cfg->in_frame.crop.height > (cfg->out_frame.crop.height << 1))))) {
		return 0;
	}
	...
	case DISP_FORMAT_A2R10G10B10: /* equal x2p10p10p10 */
		...
		break;
	default:
		DE_WARN("not support this fmt=%d\n",
...
s32 de_wb_init_rcq(u32 wb)
{
	...
	if (rcq_info->reg_blk == NULL) {
		DE_WARN("kalloc for wb reg block failed, num=%d\n", num);
		return -1;
	}
	...
	if (num != rcq_info->cur_num)
		DE_WARN("checkit: num=%d, cur_num=%d\n",
	...
	if (rcq_used) {
		...
		if (rcq_info->vir_addr == NULL) {
			DE_WARN("alloc for de_rcq_head failed\n");
			return -1;
		}
		p_reg_blks	= rcq_info->reg_blk;
...
s32 de_wb_init(struct disp_bsp_init_para *para)
{
	...
	if (wb_num > 1) {
		DE_WARN("not support more than 1 wb at present\n");
		wb_num = 1;
	}
	for (wb = 0; wb < wb_num; ++wb) {
		...
		if (reg_mem_info->vir_addr == NULL) {
			DE_WARN("alloc wb[%d] mm fail!size=0x%x\n", wb,
			       reg_mem_info->size);
			return -1;
		}
		...
		if (priv->cdc_hdl == NULL) {
			DE_WARN("de_cdc_create for wb failed\n");
		}
	...
	struct de_reg_block **blks, u32 *blk_num)
{
	...
	if (*blk_num < num) {
		DE_WARN("should not happen: blk_num=%d\n", *blk_num);
		*blk_num = 0;
		return -1;
	}
...
static int de_wb_get_enable_info(const struct dump_regs_user_setting *setting, char *buf)
{

	return 0;
}

static int de_wb_get_enable_status(const struct dump_regs_user_setting *setting)
{
	int disp = setting->index.disp;
	struct de_wb_private *priv;
	struct wb_reg __iomem *regs;
	u32 value = 0;
	u32 enable;

	if (disp >= 1) /* not support */
		return 0;

	priv = &(wb_priv[disp]);
	regs = (struct wb_reg *)(priv->reg_blks[0].reg_addr);
	de_lowlevel_ioread(&(regs->start), (void *)&value, 4);
	enable = ((union wb_start_reg)value).bits.wb_start & 0x1;

	DUMP_REGS_PRINT("wb: disp%d enable%d\n", disp, enable);

	return enable;
}

static int de_wb_dump_by_user_setting(const struct dump_regs_user_setting *setting, char *buf)
{
	unsigned int num = 0;
	struct de_wb_private *priv;
	...
	if (setting->use_index_find) {
		...
		priv = &(wb_priv[disp]);
		is_enable = de_wb_get_enable_status(setting);

		num += sprintf(buf + num, "index(%d) (%s)\n", disp, is_enable ? "enabled" : "disabled");
		if (is_enable || setting->force_dump)
			num += do_dump_regs_block(dr_wb_priv.mod, priv->reg_blks, ARRAY_SIZE(priv->reg_blks), buf + num);
		num += sprintf(buf + num, "\n");
	} else {
		...
		for (i = 0; i < screen_num; i++) {
			...
			priv = &(wb_priv[i]);
			is_enable = de_wb_get_enable_status(&msetting);
...
int de_wb_register_dr_module(void)
{
#define WB_MAX_BLOCK_SIZE (0xd0)
	struct dump_regs_module *mod = dr_module_alloc(WB_MAX_BLOCK_SIZE);;

	dr_wb_priv.mod = mod;

	mod->type = DISP_WB_BLOCK;
	strcpy(mod->name, module_name[mod->type]);
	mod->ops.dump_by_user_setting = de_wb_dump_by_user_setting;
	mod->ops.get_enable_info = de_wb_get_enable_info;
	mod->ops.get_enable_status = de_wb_get_enable_status;
	register_dr_module(mod);

	return 0;
}

int de_wb_unregister_dr_module(void)
{
	unregister_dr_module(dr_wb_priv.mod);
	return 0;
}
