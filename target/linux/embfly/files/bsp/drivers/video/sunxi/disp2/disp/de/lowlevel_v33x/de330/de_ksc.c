...
struct de_ksc_dump_regs_priv {
	struct dump_regs_module *mod;
};

struct de_ksc_dump_regs_priv dr_ksc_priv;
...
s32 de_ksc_init(u32 disp, u8 __iomem *de_reg_base)
{
	...
	if (NULL == reg_mem_info->vir_addr) {
		DE_WARN("alloc ksc[%d] mm fail!size=0x%x\n",
...
static int de_ksc_get_enable_info(const struct dump_regs_user_setting *setting, char *buf)
{

	return 0;
}

static int de_ksc_get_enable_status(const struct dump_regs_user_setting *setting)
{
	int disp = setting->index.disp;
	struct de_ksc_private *priv = &(ksc_priv[disp]);
	struct ksc_reg __iomem *regs = (struct ksc_reg *)(priv->reg_blks.reg_addr);
	u32 value = 0;
	u32 enable;

	de_lowlevel_ioread(&(regs->ksc_ctrl), (void *)&value, 4);
	enable = ((union ksc_ctrl_reg)value).bits.en & 0x1;
	DUMP_REGS_PRINT("ksc: disp%d enable%d\n", disp, enable);

	return enable;
}

static int de_ksc_dump_by_user_setting(const struct dump_regs_user_setting *setting, char *buf)
{
	unsigned int num = 0;
	struct de_ksc_private *priv;
	int i, disp, screen_num, is_enable;

	num += sprintf(buf + num, "%s module:\n\n", dr_ksc_priv.mod->name);

	if (setting->use_index_find) {
		disp = setting->index.disp;
		priv = &(ksc_priv[disp]);
		is_enable = de_ksc_get_enable_status(setting);

		num += sprintf(buf + num, "index(%d) (%s)\n", disp, is_enable ? "enabled" : "disabled");
		if (is_enable || setting->force_dump)
			num += do_dump_regs_block(dr_ksc_priv.mod, &(priv->reg_blks), 1, buf + num);
		num += sprintf(buf + num, "\n");
	} else {
		...
		for (i = 0; i < screen_num; i++) {
			...
			priv = &(ksc_priv[i]);
			is_enable = de_ksc_get_enable_status(&msetting);
...
int de_ksc_register_dr_module(void)
{
#define KSC_MAX_BLOCK_SIZE (sizeof(struct ksc_reg))
	struct dump_regs_module *mod = dr_module_alloc(KSC_MAX_BLOCK_SIZE);;

	dr_ksc_priv.mod = mod;

	mod->type = DISP_KSC_MODULE;
	strcpy(mod->name, module_name[mod->type]);
	mod->ops.dump_by_user_setting = de_ksc_dump_by_user_setting;
	mod->ops.get_enable_info = de_ksc_get_enable_info;
	mod->ops.get_enable_status = de_ksc_get_enable_status;
	register_dr_module(mod);

	return 0;
}

int de_ksc_unregister_dr_module(void)
{
	unregister_dr_module(dr_ksc_priv.mod);
	return 0;
}
