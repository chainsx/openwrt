...
static s32 disp_lyr_dump(struct disp_layer *lyr, char *buf)
{
	...
	count += sprintf(buf + count, "flags[0x%8x] metadata_flag[0x%8x] trd[%1d,%1d] ",
			 data.config.info.fb.flags, data.config.info.fb.metadata_flag,
...
disp_mgr_get_layer_cfg(struct disp_manager *mgr,
		       struct disp_layer_config2 *config)
{
	int layer_index = 0, disp, chn;
	int num_screens = bsp_disp_feat_get_num_screens();
	int num_chns = bsp_disp_feat_get_num_channels(mgr->disp);

	for (disp = 0; disp < num_screens && disp < mgr->disp; disp++)
		layer_index += bsp_disp_feat_get_num_layers(disp);

	for (chn = 0; chn < num_chns && chn < config->channel; chn++)
		layer_index += bsp_disp_feat_get_num_layers_by_chn(mgr->disp, chn);

	layer_index += config->layer_id;
...
static s32 disp_map_afbc_header(struct disp_fb_info_inner *fb)
{
	...
	if ((fb->metadata_fd < 0)
		|| !((fb->metadata_flag & SUNXI_METADATA_FLAG_AFBC_HEADER)
			...
			|| (fb->metadata_flag & SUNXI_METADATA_FLAG_HDRVIVID_HEADER))
		|| (fb->metadata_size == 0)) {
			  ...
			  unsigned int layer_num)
{
	...
	for (i = 0; i < layer_num; i++) {
		...
		/*if (lyr_cfg->config.info.fb.metadata_flag & SUNXI_METADATA_FLAG_HDRVIVID_HEADER)*/
			/*lyr_cfg->config.info.fb.metadata_flag = SUNXI_METADATA_FLAG_HDR10P_METADATA;*/
		...
		if (lyr_cfg->config.info.fb.fbd_en
			...
			if (lyr_cfg->config.info.fb.fbd_en
				&& lyr_cfg->config.info.fb.p_afbc_header)
...
static s32 disp_mgr_enable(struct disp_manager *mgr)
{
...
#if IS_ENABLED(CONFIG_AW_KSC)
	enum ksc_pix_fmt fmt = RGB888;
	switch (dev_config.format) {
	...
	ksc_online_enable(true, fmt, width, height, 8);
#endif
	return 0;
}
