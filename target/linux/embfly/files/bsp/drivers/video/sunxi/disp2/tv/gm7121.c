...
static void gm7121_init(enum disp_tv_mode tv_mode)
{
	 ...
	 */
	/* -------------------SAA7121 START------------------------------- */
	sunxi_info(NULL, "[TV]gm7121_init, tv_Mode=%d\n", tv_mode);
		...
		const struct i2c_device_id *id)
{
	...
	sunxi_info(NULL, "[DISP_I2C] tv_i2c_probe\n");
...
static int tv_i2c_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	if (tv_i2c_id == client->adapter->nr) {
		...
		sunxi_info(NULL, "[DISP_I2C] tv_i2c_detect, get right i2c adapter, id=%d\n",
			tv_i2c_adapter->nr);
...
static int __init gm7121_module_init(void)
{
	...
	sunxi_info(NULL, "[TV]gm7121_module_init begin\n");
	...
	if (ret == 1) {
		...
		if (tv_used) {
			...
			if ((output_type0 == DISP_OUTPUT_TYPE_TV) ||
				...
				sunxi_info(NULL, "[TV]%s:smooth boot", __func__);
				gm7121_tv_power_on(1);
				if (output_type0 == DISP_OUTPUT_TYPE_TV)
...
{
	sunxi_info(NULL, "gm7121_module_exit\n");
	tv_i2c_exit();
}
