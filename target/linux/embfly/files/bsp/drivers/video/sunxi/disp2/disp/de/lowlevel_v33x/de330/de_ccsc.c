...
struct de_ccsc_dump_regs_priv {
	struct dump_regs_module *mod;
};

struct de_ccsc_dump_regs_priv dr_ccsc_priv;
...
s32 de_ccsc_init(u32 disp, u8 __iomem *de_reg_base)
{
	...
	for (chn = 0; chn < chn_num; ++chn) {
		...
		if (NULL == reg_mem_info->vir_addr) {
			DE_WARN("alloc ccsc[%d] mm fail!size=0x%x\n",
...
static int de_ccsc_get_enable_info(const struct dump_regs_user_setting *setting, char *buf)
{

	return 0;
}

static int de_ccsc_get_enable_status(const struct dump_regs_user_setting *setting)
{
	int disp = setting->index.disp;
	int chn = setting->index.chn;
	struct de_ccsc_private *priv = &(ccsc_priv[disp][chn]);
	struct ccsc_reg __iomem *regs = (struct ccsc_reg *)(priv->reg_blks[0].reg_addr);
	u32 value = 0;
	u32 enable;

	de_lowlevel_ioread(&(regs->ctl), (void *)&value, 4);
	enable = ((union ccsc_ctl_reg)value).bits.en & 0x1;
	DUMP_REGS_PRINT("csc: disp%d chn%d enable%d\n", disp, chn, enable);

	return enable;
}

static int de_ccsc_dump_by_user_setting(const struct dump_regs_user_setting *setting, char *buf)
{
	unsigned int num = 0;
	struct de_ccsc_private *priv;
	int i, j, disp, chn, screen_num, chn_num, is_enable;

	num += sprintf(buf + num, "%s module:\n\n", dr_ccsc_priv.mod->name);

	if (setting->use_index_find) {
		disp = setting->index.disp;
		chn = setting->index.chn;
		priv = &(ccsc_priv[disp][chn]);
		is_enable = de_ccsc_get_enable_status(setting);

		num += sprintf(buf + num, "index(%d, %d) (%s)\n", disp, chn, is_enable ? "enabled" : "disabled");
		if (is_enable || setting->force_dump)
			num += do_dump_regs_block(dr_ccsc_priv.mod, priv->reg_blks, ARRAY_SIZE(priv->reg_blks), buf + num);
		num += sprintf(buf + num, "\n");
	} else {
		...
		for (i = 0; i < screen_num; i++) {
			...
			for (j = 0; j < chn_num; j++) {
				...
				priv = &(ccsc_priv[i][j]);
				is_enable = de_ccsc_get_enable_status(&msetting);
...
int de_ccsc_register_dr_module(void)
{
#define CSC_MAX_BLOCK_SIZE (0x40)
	struct dump_regs_module *mod = dr_module_alloc(CSC_MAX_BLOCK_SIZE);

	dr_ccsc_priv.mod = mod;

	mod->type = DISP_CSC_MODULE;
	strcpy(mod->name, module_name[mod->type]);
	mod->ops.dump_by_user_setting = de_ccsc_dump_by_user_setting;
	mod->ops.get_enable_info = de_ccsc_get_enable_info;
	mod->ops.get_enable_status = de_ccsc_get_enable_status;
	register_dr_module(mod);

	return 0;
}

int de_ccsc_unregister_dr_module(void)
{
	unregister_dr_module(dr_ccsc_priv.mod);
	return 0;
}
