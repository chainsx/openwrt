 ...
 */
int de_dcsc_coeff_calc(unsigned int infmt, unsigned int incscmod,
			    ...
			    bool use_user_matrix)
{
	...
	if (brightness != 50 || contrast != 50 || saturation != 50 ||
		...
		if (incscmod == outcscmod && infmt == DE_YUV && outfmt == DE_YUV) {
			i_bright = i_bright >> 1;
...
int de_dcsc_apply(unsigned int sel, struct disp_csc_config *config)
{
	...
	DE_INFO("sel=%d, in_fmt=%d, mode=%d, out_fmt=%d, mode=%d, range=%d use_user_matrix=%d\n",
	      sel, config->in_fmt, config->in_mode, config->out_fmt,
	      config->out_mode, config->out_color_range, use_user_matrix);
	...
	de_gamma_set_csc(sel, 0, DISP_GAMMA_TYPE, 1, csc_coeff, ctx->output.data_bits);
