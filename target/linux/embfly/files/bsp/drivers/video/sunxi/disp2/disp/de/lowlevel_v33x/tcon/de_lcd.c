...
struct de_lcd_dump_regs_priv {
	struct dump_regs_module *mod;
};

struct de_lcd_dump_regs_priv dr_lcd_priv;
...
#define TCON_MAX_BLOCK_SIZE (0x240)
static int de_lcd_get_enable_info(const struct dump_regs_user_setting *setting, char *buf)
{
	return 0;
}

static int de_lcd_get_enable_status(const struct dump_regs_user_setting *setting)
{
	return 0;
}

static int de_lcd_dump_by_user_setting(const struct dump_regs_user_setting *setting, char *buf)
{
	unsigned int i, num = 0, is_enable;
	int disp = setting->index.disp;
	int tcon_index;
	struct disp_manager *mgr;
	u32 *value = (u32 *)dr_lcd_priv.mod->par;
	u8 __iomem *addr_base;
	int addr_offset = 0;

	mgr = disp_get_layer_manager(disp);
	if (mgr && mgr->device) {
		tcon_index = mgr->device->hwdev_index;
		addr_base = (u8 __iomem *)tcon_get_reg_base(tcon_index);
		is_enable = 1;
	} else {
		tcon_index = 0;
		addr_base = (u8 __iomem *)tcon_get_reg_base(tcon_index);
	...
	DE_WARN("tcon: disp%d tcon_index%d is_enable%d\n", disp, tcon_index, is_enable);

	num += sprintf(buf + num, "%s module:\n\n", dr_lcd_priv.mod->name);

	de_lowlevel_ioread(addr_base, (void *)value, TCON_MAX_BLOCK_SIZE);
	for (i = 0; i < TCON_MAX_BLOCK_SIZE / 4; i += 4) {
	...
	addr_base = (u8 __iomem *)tcon_top_get_reg_base(0);
	num += sprintf(buf + num, "%s module:\n\n", "tcon top");

	de_lowlevel_ioread(addr_base, (void *)value, 0x100);
	for (i = 0; i < 0x100 / 4; i += 4) {
			num += sprintf(buf + num, "0x%04x: %08x %08x %08x %08x\n", addr_offset + i * 4,
				       value[i], value[i + 1], value[i + 2], value[i + 3]);
	}
	num += sprintf(buf + num, "\n");
...
int de_lcd_register_dr_module(void)
{
	struct dump_regs_module *mod = dr_module_alloc(TCON_MAX_BLOCK_SIZE);

	dr_lcd_priv.mod = mod;

	mod->type = DISP_TCON_MODULE;
	strcpy(mod->name, module_name[mod->type]);
	mod->ops.dump_by_user_setting = de_lcd_dump_by_user_setting;
	mod->ops.get_enable_info = de_lcd_get_enable_info;
	mod->ops.get_enable_status = de_lcd_get_enable_status;
	register_dr_module(mod);

	return 0;
}

int de_lcd_unregister_dr_module(void)
{
	unregister_dr_module(dr_lcd_priv.mod);
	return 0;
}
