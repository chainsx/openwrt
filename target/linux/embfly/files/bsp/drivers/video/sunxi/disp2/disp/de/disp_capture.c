...
struct disp_capture *disp_get_capture(u32 disp)
{
	...
	if (!bsp_disp_feat_is_support_capture(disp)) {
		DE_INFO("screen %d not support capture\n", disp);
		return NULL;
	}
							       ...
							       *cptr)
{
	...
	if (!bsp_disp_feat_is_support_capture(cptr->disp)) {
		DE_WARN("screen %d not support capture\n", cptr->disp);
		return NULL;
	}
...
s32 disp_capture_start(struct disp_capture *cptr, struct disp_capture_init_info *info)
{
	struct disp_capture_private_data *cptrp = disp_capture_get_priv(cptr);
	...
	if (cptrp->enabled == 1) {
		DE_WARN("capture %d already started\n", cptr->disp);
	...
	disp_al_capture_init(cptr->disp, info->port);
			...
			struct disp_capture_info *info)
{
	...
	if ((DISP_CSC_TYPE_RGB != cs) &&
		...
		DE_WARN("in_fmt and out_fmt not match\n");
		return -1;
	}
#endif
	...
	if ((DISP_CSC_TYPE_YUV444 == cs) || (DISP_CSC_TYPE_YUV422 == cs) ||
		...
		if ((info->out_frame.format != DISP_FORMAT_YUV420_P) &&
			...
			DE_WARN("out_format is not support\n");
			return -1;
		}
	}
#endif
...
s32 disp_capture_init(struct disp_capture *cptr)
{
	...
	if (!bsp_disp_feat_is_support_capture(cptr->disp)) {
		DE_WARN("capture %d is not support\n", cptr->disp);
		return -1;
	}
