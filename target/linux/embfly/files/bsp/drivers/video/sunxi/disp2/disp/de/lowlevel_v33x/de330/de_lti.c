...
struct de_lti_dump_regs_priv {
	struct dump_regs_module *mod;
};

struct de_lti_dump_regs_priv dr_lti_priv;
			   ...
			   u32 bypass)
{
	...
	if (para->level >= LTI_PARA_NUM || dev_type >= 2) {
		DE_WARN("lti para over range level %d dev_type %d",
			 para->level, dev_type);
		return -1;
	}
...
static int de_lti_get_enable_info(const struct dump_regs_user_setting *setting, char *buf)
{

	return 0;
}

static int de_lti_get_enable_status(const struct dump_regs_user_setting *setting)
{
	int disp = setting->index.disp;
	int chn = setting->index.chn;
	struct de_lti_private *priv = &(lti_priv[disp][chn]);
	struct lti_reg __iomem *regs = (struct lti_reg *)(priv->reg_blks[0].reg_addr);
	u32 value = 0;
	u32 enable;
	...
	enable = ((union lti_en)value).bits.en & 0x1;
	DUMP_REGS_PRINT("lti: disp%d chn%d enable%d\n", disp, chn, enable);

	return enable;
}

static int de_lti_dump_by_user_setting(const struct dump_regs_user_setting *setting, char *buf)
{
	unsigned int num = 0;
	struct de_lti_private *priv;
	int i, j, disp, chn, screen_num, chn_num, is_enable, is_support;

	num += sprintf(buf + num, "%s module:\n\n", dr_lti_priv.mod->name);

	if (setting->use_index_find) {
		disp = setting->index.disp;
		chn = setting->index.chn;
		priv = &(lti_priv[disp][chn]);
		is_enable = de_lti_get_enable_status(setting);
		...
		if ((is_enable || setting->force_dump) && is_support)
			num += do_dump_regs_block(dr_lti_priv.mod, priv->reg_blks, ARRAY_SIZE(priv->reg_blks), buf + num);
		num += sprintf(buf + num, "\n");
	} else {
		...
		for (i = 0; i < screen_num; i++) {
			...
			for (j = 0; j < chn_num; j++) {
				...
				priv = &(lti_priv[i][j]);
				is_enable = de_lti_get_enable_status(&msetting);
...
int de_lti_register_dr_module(void)
{
#define LTI_MAX_BLOCK_SIZE (0x44)
	struct dump_regs_module *mod = dr_module_alloc(LTI_MAX_BLOCK_SIZE);

	dr_lti_priv.mod = mod;

	mod->type = DISP_LCTI_MODULE;
	strcpy(mod->name, module_name[mod->type]);
	mod->ops.dump_by_user_setting = de_lti_dump_by_user_setting;
	mod->ops.get_enable_info = de_lti_get_enable_info;
	mod->ops.get_enable_status = de_lti_get_enable_status;
	register_dr_module(mod);

	return 0;
}

int de_lti_unregister_dr_module(void)
{
	unregister_dr_module(dr_lti_priv.mod);
	return 0;
}
