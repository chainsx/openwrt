...
static u8 reg_bank = 1;

static struct dentry *gHdmi_debugfs_dir;
...
static ssize_t hdmi_reg_dump_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t n = 0;
	int i = 0;
...
ssize_t hdmi_reg_dump_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	char *end;
...
static DEVICE_ATTR(reg_dump, 0664,
		hdmi_reg_dump_show, hdmi_reg_dump_store);

static unsigned long start_reg;
static unsigned long read_count;
static ssize_t hdmi_reg_read_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t n = 0;
	int i, j;
	...
	n += sprintf(buf + n, "[%s] start_reg: 0x%lx count: %ld\n",
			reg_bank_msg[reg_bank].msg, start_reg, read_count);
	...
	for (i = start_reg; i < start_reg + read_count; i++) {
		...
		switch (reg_bank) {
		case HDMI2_BASIC_REG:
			n += sprintf(buf + n, " 0x%02x", aw_hdmi_drv_read(i * 4));
			break;
		case HDMI2_PHY_REG:
			core->phy_ops.phy_read((u16)i, &value);
#if defined(SUNXI_HDMI20_PHY_SNPS)
			n += sprintf(buf + n, " 0x%04x", value);
#else
			n += sprintf(buf + n, " 0x%02x", value);
		...
		case HDMI2_SCDC_REG:
			core->dev_ops.scdc_read((u8)i, 1, (u8 *)&value);
			n += sprintf(buf + n, " 0x%02x", value);
			break;
#endif
#if IS_ENABLED(CONFIG_AW_HDMI2_HDCP22_SUNXI)
		case HDMI2_HPI_REG:
			value = *((u32 *)(core->mode.pHdcp.esm_hpi_base + i));
			n += sprintf(buf + n, " 0x%04x", value);
			break;
#endif
		case HDMI2_NONE_REG:
		default:
			n += sprintf(buf + n, "\nnot support read from reg_bank: %d!\n",
					reg_bank);
			return n;
		}
	}
...
ssize_t hdmi_reg_read_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	u8 *separator;
...
static DEVICE_ATTR(reg_read, 0664,
		hdmi_reg_read_show, hdmi_reg_read_store);

static ssize_t hdmi_reg_write_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t n = 0;
	n += sprintf(buf + n, "[Usage]\n");
	n += sprintf(buf + n, "    echo offset count > reg_write\n");
	n += sprintf(buf + n, "[Params]\n");
	n += sprintf(buf + n, "    offset: register offset address\n");
...
ssize_t hdmi_reg_write_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
...
cmd_write:
	hdmi_inf("[%s] write reg: 0x%lx value: %lx\n",
			reg_bank_msg[reg_bank].msg, reg_addr, value);
	...
	case HDMI2_NONE_REG:
	default:
		hdmi_err("%s: not support write to reg_bank: %d!\n", __func__, reg_bank);
		return -EINVAL;
	}
...
static DEVICE_ATTR(reg_write, 0664,
		hdmi_reg_write_show, hdmi_reg_write_store);

static ssize_t hdmi_reg_bank_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int i;
	ssize_t n = 0;
	n += sprintf(buf + n, "[Usage]\n");
	n += sprintf(buf + n, "    echo index > reg_bank\n");
	n += sprintf(buf + n, "[Params]\n");
	for (i = 1; i < ARRAY_SIZE(reg_bank_msg); i++) {
		n += sprintf(buf + n, "    %d: %s\n",
				reg_bank_msg[i].code, reg_bank_msg[i].msg);
	}
	n += sprintf(buf + n, "\n[Current Setting]\n");
	n += sprintf(buf + n, "    %d: %s\n",
			reg_bank_msg[reg_bank].code, reg_bank_msg[reg_bank].msg);
	return n;
}

static ssize_t hdmi_reg_bank_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	reg_bank = (u8)simple_strtoull(buf, NULL, 0);

	if (reg_bank >= ARRAY_SIZE(reg_bank_msg)) {
		hdmi_err("%s: reg_bank: %d error!\n", __func__, reg_bank);
		reg_bank = 1;
		return -EINVAL;
	}

#if !IS_ENABLED(CONFIG_AW_HDMI2_HDCP22_SUNXI)
	if (reg_bank == HDMI2_HPI_REG) {
		hdmi_err("%s: not support HDCP2.2!\n", __func__);
		reg_bank = 1;
		return -EINVAL;
	}
#endif

	hdmi_inf("set reg_bank to %s\n", reg_bank_msg[reg_bank].msg);

	return count;
}

static DEVICE_ATTR(reg_bank, 0664,
		hdmi_reg_bank_show, hdmi_reg_bank_store);
...
{
	/*  hdcp_type:
	 *      Bit  7:0: The hdcp type currently in use.
	 *      Bit 31:8: The hdcp type supported by the driver.
	 */
	u32 hdcp_type = aw_hdmi_drv_get_hdcp_type(g_hdmi_drv->hdmi_core) & 0xFF;


	if (g_hdmi_drv->hdmi_core->mode.pHdcp.use_hdcp) {
		hdcp_type |= BIT(AW_HDCP_TYPE_HDCP14 + 8);

		if (g_hdmi_drv->hdmi_core->mode.pHdcp.use_hdcp22)
			hdcp_type |= BIT(AW_HDCP_TYPE_HDCP22 + 8);
	}

	mutex_lock(&g_hdmi_drv->aw_mutex.lock_hdcp);
	memcpy(buf, &hdcp_type, sizeof(hdcp_type));
	mutex_unlock(&g_hdmi_drv->aw_mutex.lock_hdcp);

	return sizeof(hdcp_type);
}
...
static ssize_t hdmi_hdcp_dump_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
...
static ssize_t _aw_hdmi_sink_edid_parse(char *buf, ssize_t n)
{
	...
	/* dump standard timing */
	n += sprintf(buf + n, "\n");
	for (i = 0; i < sink_cap->edid_mStandardIndex; i++) {
		n += sprintf(buf + n, "\n\t\tVESA_%dx%dP%d",
			sink_cap->edid_mStandard[i].hactive,
			sink_cap->edid_mStandard[i].vactive,
			sink_cap->edid_mStandard[i].rate);
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (temp == 0)
		temp = g_hdmi_drv->hdmi_core->video_ops.get_vic_code_sw();
...
static ssize_t hdmi_setting_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t n = 0;

	n += sprintf(buf + n, "[Example]\n");
	n += sprintf(buf + n, "    echo avmute 1 > setting\n");
	n += sprintf(buf + n, "    echo ddc 100 100 > setting\n");

	n += sprintf(buf + n, "\n[Current Setting]\n");
	n += sprintf(buf + n, "    avmute   : %d    (0: Disable 1: Enable)\n",
			g_hdmi_drv->hdmi_core->video_ops.get_avmute());
	n += sprintf(buf + n, "    dvi      : %d    (0: HDMI 1: DVI)\n",
			!g_hdmi_drv->hdmi_core->video_ops.get_tmds_mode());
	n += sprintf(buf + n, "    phy_power: %d    (0: Disable 1: Enable)\n",
			g_hdmi_drv->hdmi_core->phy_ops.phy_get_power());
	n += sprintf(buf + n, "    ddc      : %d %d (<scl_low_time> <scl_high_time>)\n",
			i2c_min_ss_scl_low_time,
			i2c_min_ss_scl_high_time);
#if IS_ENABLED(CONFIG_AW_HDMI2_LOG_BUFFER)
	n += sprintf(buf + n, "    log      : %d    (0: Disable 1: Enable)\n",
...
static ssize_t hdmi_setting_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int value = 0;
	char *end;
	...
	if (strncmp(buf, "avmute", 6) == 0) {
		value = (u8)simple_strtoull(buf + 7, NULL, 0);
		hdmi_inf("set avmute to %s\n", value ? "enable" : "disable");
	...
	} else if (strncmp(buf, "dvi", 3) == 0) {
		value = (u8)simple_strtoull(buf + 4, NULL, 0);
		hdmi_inf("set tmds to %s\n", value ? "DVI" : "HDMI");
	...
	} else if (strncmp(buf, "phy_power", 9) == 0) {
		value = (u8)simple_strtoull(buf + 10, NULL, 0);
		hdmi_inf("set phy_power to %s\n", value ? "enable" : "disable");
	...
	} else if (strncmp(buf, "ddc", 3) == 0) {

		i2c_min_ss_scl_low_time = (u16)simple_strtoull(buf + 4, &end, 0);
		...
		hdmi_inf("set ddc to low: %d high: %d\n", i2c_min_ss_scl_low_time,
				i2c_min_ss_scl_high_time);
#if IS_ENABLED(CONFIG_AW_HDMI2_LOG_BUFFER)
	} else if (strncmp(buf, "log", 3) == 0) {
		value = (u8)simple_strtoull(buf + 4, NULL, 0);
		hdmi_inf("set hdmi_log to %s\n", value ? "enable" : "disable");

		aw_hdmi_log_set_enable(!!value);
#endif
	} else {
		hdmi_err("%s: cmds not support now!!!\n", __func__);
	}

	return count;
}

static DEVICE_ATTR(setting, 0664,
		hdmi_setting_show, hdmi_setting_store);

static ssize_t hdmi_pattern_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int n = 0;
	int i;
	...
	n += sprintf(buf + n, "[Usage]\n");
	n += sprintf(buf + n, "    echo index > pattern\n");
	n += sprintf(buf + n, "[Pattern]\n");
	for (i = 0; i < ARRAY_SIZE(pattern_msg); i++) {
		n += sprintf(buf + n, "    %d: %s\n",
				pattern_msg[i].code, pattern_msg[i].msg);
	}

	return n;
}

ssize_t hdmi_pattern_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int bit = 0;
	char *end;
	...
	if ((bit < 0) || (bit >= ARRAY_SIZE(pattern_msg))) {
		hdmi_err("%s: input index %d unsupport\n", __func__, bit);
		return -EINVAL;
	}

	if (bit == 0) {
		g_hdmi_drv->hdmi_core->video_ops.show_pattern(0x0, 0x0);
		hdmi_inf("disable force video output\n");
		return count;
	}

	g_hdmi_drv->hdmi_core->video_ops.show_pattern(0x1, value[bit - 1]);

	return count;
}

static DEVICE_ATTR(pattern, 0664,
		hdmi_pattern_show, hdmi_pattern_store);

static ssize_t hdmi_audio_dump_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	ssize_t n = 0;

	n += g_hdmi_drv->hdmi_core->audio_ops.audio_dump(buf + n);

	return n;
}

static DEVICE_ATTR(audio_dump, 0444,
		hdmi_audio_dump_show, NULL);
...
#if IS_ENABLED(CONFIG_AW_HDMI2_LOG_BUFFER)
static void *_aw_hdmi_seq_log_start(struct seq_file *s, loff_t *pos)
{
	char *addr = aw_hdmi_log_get_address();
	unsigned int start = aw_hdmi_log_get_start_index();
	unsigned int max_size = aw_hdmi_log_get_max_size();
	...
	/* *pos < used_size <= max_size */
	if (*pos >= used_size)
		return NULL;

	if ((start + *pos) >= max_size)
		return addr + (*pos - (max_size - start));
	else
		return addr + (start + *pos);
}

static void _aw_hdmi_seq_log_stop(struct seq_file *s, void *v)
{
	aw_hdmi_log_put_address();
}

static void *_aw_hdmi_seq_log_next(struct seq_file *s, void *v, loff_t *pos)
{
	char *addr = (char *)v;
	unsigned int start = aw_hdmi_log_get_start_index();
	unsigned int max_size = aw_hdmi_log_get_max_size();
	...
	if ((start + *pos) == (max_size - 1))
		/* loop addr to the beginning of memory */
		addr -= (max_size - 1);
	else
		addr += 1;
...
static int _aw_hdmi_seq_log_show(struct seq_file *s, void *v)
{
	seq_printf(s, "%c", *(char *)v);
	return 0;
}

static const struct seq_operations aw_hdmi_log_seqops = {
	.start = _aw_hdmi_seq_log_start,
	.next  = _aw_hdmi_seq_log_next,
	.stop  = _aw_hdmi_seq_log_stop,
	.show  = _aw_hdmi_seq_log_show,
};

static int _aw_hdmi_seq_log_open(struct inode *inode, struct file *file)
{
	return seq_open(file, &aw_hdmi_log_seqops);
}

static const struct file_operations aw_hdmi_log_fops = {
	.open = _aw_hdmi_seq_log_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = seq_release,
};
...
static int __init aw_hdmi_module_init(void)
{
	...
	gHdmi_debugfs_dir = debugfs_create_dir("hdmi", NULL);
	if (!gHdmi_debugfs_dir) {
		hdmi_wrn("debugfs create dir failed!\n");
	} else {
#if IS_ENABLED(CONFIG_AW_HDMI2_LOG_BUFFER)
		debugfs_create_file("hdmi_log", 0440, gHdmi_debugfs_dir, NULL, &aw_hdmi_log_fops);
#endif
	}
...
{
	debugfs_remove(gHdmi_debugfs_dir);
	gHdmi_debugfs_dir = NULL;
