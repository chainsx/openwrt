...
#define DE_VER_CTL_OFFSET            (0x0)
#define DE_VER_TIME_OFFSET           (0x4)
#define DE_IP_CONFIG_OFFSET          (0x8)
#define DE_AHB_RESET_OFFSET          (0x10)
#define DE_MOD_ENABLE_OFFSET         (0x14)
#define DE_MBUS_CLOCK_OFFSET         (0x18)
#define DE_GLOBAL_CLOCK_OFFSET       (0x1C)
#define DE_RTWB_CTL_OFFSET           (0x20)
#define DE_RTWB_MUX_OFFSET           (0x24)
#define DE2TCON_MUX_OFFSET           (0x28)
#define DE_RESERVE_CTL_OFFSET        (0x2C)
#define DE_AHB_TIMEOUT_OFFSET        (0x30)
#define DE_DEBUG_CTL_OFFSET          (0x34)

#define RTMX_GLB_CTL_OFFSET(disp)              (0x100 + (disp) * 0x40)
#define RTMX_GLB_STS_OFFSET(disp)              (0x104 + (disp) * 0x40)
#define RTMX_GLB_SIZE_OFFSET(disp)             (0x108 + (disp) * 0x40)
#define RTMX_RCQ_CTL_OFFSET(disp)              (0x110 + (disp) * 0x40)
#define RTMX_RCQ_STATUS_OFFSET(disp)           (0x114 + (disp) * 0x40)
#define RTMX_RCQ_HEADER_LADDR_OFFSET(disp)     (0x118 + (disp) * 0x40)
#define RTMX_RCQ_HEADER_HADDR_OFFSET(disp)     (0x11C + (disp) * 0x40)
#define RTMX_RCQ_HEADER_LENGTH_OFFSET(disp)    (0x120 + (disp) * 0x40)
...
static struct de_clk_para de_clk_tbl[] = {
	{
		.clk_no             = DE_CLK_CORE0,
		.ahb_reset_adr      = DE_AHB_RESET_OFFSET,
		.ahb_reset_shift    = 0,
		.ahb_reset_width    = 1,
		.mod_en_adr         = DE_MOD_ENABLE_OFFSET,
		.mod_en_shift       = 0,
		.mod_en_width       = 1,
	},
	{
...
static void __de_mbus_clk_enable(u8 en)
{
	...
	writel(reg_val, de_base + DE_MBUS_CLOCK_OFFSET);
}
...
{
	writel((readl(de_base + DE_RTWB_MUX_OFFSET) & 0xfffffffc) | id,
...
enum de_rtwb_mux_id de_top_get_rtwb_mux(u32 wb_id)
{
	...
	return (enum de_rtwb_mux_id)(reg_val & 0x3);
}

s32 de_top_set_rtwb_mode(u32 wb_id, enum de_rtwb_mode mode)
{
	if (mode == TIMING_FROM_TCON)
		writel(readl(de_base + DE_RTWB_CTL_OFFSET) & 0xFFFFFFEF,
		       de_base + DE_RTWB_CTL_OFFSET);
	else
		writel(readl(de_base + DE_RTWB_CTL_OFFSET) | 0x10,
		       de_base + DE_RTWB_CTL_OFFSET);
...
{
	if (en)
		writel(readl(de_base + DE_RTWB_CTL_OFFSET) | 0x1,
		       de_base + DE_RTWB_CTL_OFFSET);
	else
		writel(readl(de_base + DE_RTWB_CTL_OFFSET) & 0XFFFFFFFE,
		       de_base + DE_RTWB_CTL_OFFSET);
	return 0;
}
...
s32 de_top_set_rtmx_enable(u32 disp, u8 en)
{
	...
	reg_base = de_base + RTMX_GLB_CTL_OFFSET(hw_disp);
	reg_val = readl(reg_base);
	...
	width = 1;
	shift = hw_disp << 2;
	reg_base = de_base + DE_GLOBAL_CLOCK_OFFSET;
	reg_val = readl(reg_base);
	reg_val = SET_BITS(shift, width, reg_val, en & 0x1);
	writel(reg_val, reg_base);
	return 0;
...
{
	u32 hw_disp = de_feat_get_hw_disp(disp);
	u8 __iomem *reg_base = de_base + RTMX_GLB_SIZE_OFFSET(hw_disp);
...
s32 de_top_rcq_set_enable(u32 disp, u8 en)
{
	/* u32 hw_disp = de_feat_get_hw_disp(disp); */
	u8 __iomem *reg_base;
	u32 reg_val;
...
s32 de_top_enable_ksc(u32 disp, u8 en)
{
	u8 __iomem *reg_base;
	u32 reg_val;
...
s32 de_top_rcq_enable_irq(u32 disp, u32 irq_flag, u32 en)
{
	s32 hw_disp = de_feat_get_hw_disp(disp);
...
u32 de_top_rcq_query_state_with_clear(u32 disp, u32 irq_state)
{
	s32 hw_disp = de_feat_get_hw_disp(disp);
	u8 __iomem *reg_base = de_base + RTMX_RCQ_STATUS_OFFSET(hw_disp);
	u32 reg_val = readl(reg_base);
	u32 state = reg_val & irq_state & DE_RCQ_IRQ_STATE_MASK;

	reg_val &= ~DE_RCQ_IRQ_STATE_MASK;
	reg_val |= state;
	writel(reg_val, reg_base); /* w1c */
...
u32 de_top_rcq_query_state_is_busy(u32 disp)
{
	s32 hw_disp = de_feat_get_hw_disp(disp);
	u8 __iomem *reg_base = de_base + RTMX_RCQ_STATUS_OFFSET(hw_disp);
	u32 reg_val = readl(reg_base);
	u32 state = reg_val & 0x100;
...
s32 de_top_set_rcq_update(u32 disp, u32 update)
{
	...
	reg_base = de_base + RTMX_RCQ_CTL_OFFSET(hw_disp);
	reg_val = readl(reg_base);
	reg_val = SET_BITS(shift, width, reg_val, update & 0x1);
	writel(reg_val, reg_base);
	return 0;
}
...
s32 de_top_set_rcq_head(u32 disp, u64 addr, u32 len)
{
	...
	reg_base = de_base + RTMX_RCQ_HEADER_LADDR_OFFSET(hw_disp);
	writel((u32)addr, reg_base);
	reg_base = de_base + RTMX_RCQ_HEADER_HADDR_OFFSET(hw_disp);
	writel(haddr, reg_base);
	reg_base = de_base + RTMX_RCQ_HEADER_LENGTH_OFFSET(hw_disp);
	writel(len, reg_base);
	return 0;
}
...
#define DE_BLOCK_SIZE 262144 /* 64 * 4094 */
