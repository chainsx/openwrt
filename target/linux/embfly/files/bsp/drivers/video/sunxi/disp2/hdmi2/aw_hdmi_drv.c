...
#else
	int i;
	const char *disp_param;
	char name[32] = {0};

	for (i = 0; i < 2; i++) {
		sprintf(name, "boot_disp%d", i);
		disp_param = disp_boot_para_parse_str(name);
		if (disp_param && DISP_OUTPUT_TYPE_HDMI == simple_strtoull(disp_param, NULL, 0))
			return true;
		else
			hdmi_wrn("config %s not found hdmi!\n", name);
	}
...
static s32 _aw_disp_hdmi_suspend(void)
{
	...
	{
#endif
		/* Only need to close when in deep sleep */
		if (g_hdmi_drv->hdmi_core->mode.pHdcp.use_hdcp)
			g_hdmi_drv->hdmi_core->hdcp_ops.hdcp_close();
...
#if IS_ENABLED(CONFIG_PM_RUNTIME) && IS_ENABLED(CONFIG_AW_HDMI2_CEC_SUNXI) &&\
		IS_ENABLED(CONFIG_ARCH_SUN55IW3)
		pm_runtime_put_sync(g_hdmi_drv->parent_dev);
#endif
	}
...
#if IS_ENABLED(CONFIG_PM_RUNTIME) && IS_ENABLED(CONFIG_AW_HDMI2_CEC_SUNXI) &&\
		IS_ENABLED(CONFIG_ARCH_SUN55IW3)
		pm_runtime_get_sync(g_hdmi_drv->parent_dev);
#endif
		_aw_hdmi_set_tcon_pad(true);
...
static s32 _aw_disp_hdmi_get_static_config(struct disp_device_config *config)
{
	...
	config->hdr_type = g_hdmi_drv->hdmi_core->config.hdr_type;
...
static s32 _aw_disp_hdmi_set_vsif_config(void *config, struct disp_device_dynamic_config *scfg)
{
	if (g_hdmi_drv->hdmi_core == NULL) {
		hdmi_err("%s: hdmi_core is NULL!\n", __func__);
		return -1;
	}
	...
	if (config == NULL) {
		if (is_vsif < 5) {
			...
			if (g_hdmi_drv->hdmi_core->video_ops.set_vsif_config(config, scfg)) {
				hdmi_err("%s: set vsif failed!!!\n", __func__);
				return -1;
			}
		}
		if (is_vsif < 10)
...
void _aw_hdmi_init_disp_func(void)
{
	...
	disp_func.set_vsif_config        = _aw_disp_hdmi_set_vsif_config;
...
static irqreturn_t _aw_hdmi_cec_irq_handler(int irq, void *data)
{
	...
	if (stat == 0) {
		...
		/* The hpd interrupt mask may be cleared during the suspend and resume,
		 * which may cuase `irq xxx: nobody cared`.
		 */
		stat = g_hdmi_drv->hdmi_core->phy_ops.get_ir_state();
		if (stat) {
			g_hdmi_drv->hdmi_core->phy_ops.clear_ir_state(stat);
			g_hdmi_drv->hdmi_core->phy_ops.irq_all_mask();
			ret = IRQ_HANDLED;
		}
...
int aw_hdmi_cec_suspend(struct device *dev)
{
	...
	gHdmi_cec_log_addr = g_hdmi_drv->hdmi_core->cec_ops.get_la();

	g_hdmi_drv->hdmi_core->dev_ops.dev_irq_all_mute(1);
...
#if IS_ENABLED(CONFIG_PM_RUNTIME) && IS_ENABLED(CONFIG_ARCH_SUN55IW3)
		pm_runtime_put_sync(g_hdmi_drv->parent_dev);
#endif
	}
...
int aw_hdmi_cec_resume(struct device *dev)
{
	...
	g_hdmi_drv->hdmi_core->cec_ops.set_la(gHdmi_cec_log_addr);

	/* mute all irq */
	g_hdmi_drv->hdmi_core->dev_ops.dev_irq_all_mute(1);
	/* unmute cec irq */
	g_hdmi_drv->hdmi_core->dev_ops.dev_irq_all_mute(0);
...
ssize_t aw_hdmi_cec_dump_info(char *buf)
{
	ssize_t n = 0;

	n += sprintf(buf + n, "\n[cec]\n");
	n += sprintf(buf + n, " - status: %s\n", bHdmi_cec_enable ? "enable" : "disable");
	n += sprintf(buf + n, " - suspend: %d\n", bHdmi_cec_suspend_enable);
#ifdef TCON_PAN_SEL
	n += sprintf(buf + n, " - tcon pad: %s\n", _aw_hdmi_get_tcon_pad() ? "set" : "unset");
#endif
	n += sprintf(buf + n, " - logical address: 0x%04x\n",
			g_hdmi_drv->hdmi_core->cec_ops.get_la());
	n += sprintf(buf + n, " - physical address: 0x%04x\n",
			g_hdmi_drv->hdmi_core->cec_ops.get_pa());
	n += sprintf(buf + n, "\n");
