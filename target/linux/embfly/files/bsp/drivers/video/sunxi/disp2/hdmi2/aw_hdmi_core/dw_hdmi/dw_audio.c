...
static u32 _dw_audio_get_clock_cts(dw_hdmi_dev_t *dev)
{
	u32 value = 0x0;
	value |= (dev_read(dev, AUD_CTS1) & AUD_N1_AUDN_MASK);
	value |= (dev_read(dev, AUD_CTS2) & AUD_N2_AUDN_MASK) << 8;
	value |= (dev_read_mask(dev, AUD_CTS3, AUD_CTS3_AUDCTS_MASK)) << 16;
	return value;
}

static u8 _dw_audio_get_clock_cts_mode(dw_hdmi_dev_t *dev)
{
	return dev_read_mask(dev, AUD_CTS3, AUD_CTS3_CTS_MANUAL_MASK);
}
...
u8 dw_mc_get_audio_sample_clock(dw_hdmi_dev_t *dev)
{
	return (u8)dev_read_mask(dev, MC_CLKDIS, MC_CLKDIS_AUDCLK_DISABLE_MASK);
}
...
ssize_t dw_audio_dump(dw_hdmi_dev_t *dev, char *buf)
{
	...
	if (dev->snps_hdmi_ctrl.hdmi_on == 0) {
		n += sprintf(buf + n, " - Warn: dvi mode so not set audio!\n");
		return n;
	}

	n += sprintf(buf + n, " - audio clock: %s\n",
			dw_mc_get_audio_sample_clock(dev) ? "on" : "off");
	n += sprintf(buf + n, " - fs: %dHz, factor: %d, mute: %s\n",
			dw_fc_audio_get_sample_freq(dev), _dw_audio_get_fs_factor(dev),
			dw_fc_audio_get_mute(dev) ? "on" : "off");
	n += sprintf(buf + n, " - acr_n: %d, acr_cts: %d, cts_mode: %s\n",
			dw_audio_get_clock_n(dev), _dw_audio_get_clock_cts(dev),
			_dw_audio_get_clock_cts_mode(dev) ? "user" : "auto");
