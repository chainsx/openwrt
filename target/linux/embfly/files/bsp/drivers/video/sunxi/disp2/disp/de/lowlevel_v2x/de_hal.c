		...
		struct scaler_para *ovl_cpara)
{
	...
	for (j = 0; j < chn; j++) {
		...
		DE_INFO("ovl_rect[%d]=<%d,%d>\n", j, ovlw[j], ovlh[j]);
		DE_INFO("bld_rect[%d]=<%d,%d,%d,%d>\n", j, bld_rect[j].x,
		      bld_rect[j].y, bld_rect[j].w, bld_rect[j].h);
	}

	/* get video overlay parameter for scaler */
	for (j = 0; j < vi_chn; j++) {
		DE_INFO("fmt[%d]=%d, ovlw[%d]=%d,ovlh[%d]=%d, bld_rect[%d].w=%d, bld_rect[%d].h=%d\n",
		...
		for (i = 0; i < layno; i++) {
			if (lay_en[j][i] &&
				...
				if (!de_feat_is_support_scale_by_chn(screen_id, j)) {
					DE_ERR("channal%d not support scale, "
...
/* check the video format for fill color, because of the hardware limit */
	...
	for (i = 0; i < chn; i++) {
		DE_INFO("sel=%d, pipe_rect[%d]=<%d,%d,%d,%d>\n", screen_id, i,
	...
	/* set enhance size */
	/* dcsc set size */
	de_dcsc_set_size(screen_id, dispsize[0].width, dispsize[0].height);
