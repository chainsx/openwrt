...
#endif
	encode_config enc_conf[MAX_SENSOR_NUM];
};
	...
	PARAM_IN comp_buffer_header_type *pBuffer)
{
	...
	RT_LOGI("set recoder->enc_yuv_phy_addr: %p", recoder->enc_yuv_phy_addr);
	return 0;
}
...
static void rt_tdm_buffer_done_callback(struct vin_isp_tdm_event_status *status)
{
	...
	//RT_LOGW("tdm%d iommu_buf %p buf_size %d buf_id %d", status->dev_id, status->iommu_buf, status->buf_size, status->buf_id);
...
static void rt_tdm_get_buf(struct vin_isp_tdm_event_status *status)
{
	...
	//RT_LOGW("tdm%d get buf[%d] %p %d", status->dev_id, status->buf_id, status->iommu_buf, status->buf_size);
...
static void fill_venc_config(venc_comp_base_config *pvenc_config, video_recoder *recoder)
{
	...
	memcpy(&pvenc_config->s_crop_info, &media_config->s_crop_info, sizeof(RTCropInfo));
	memcpy(&pvenc_config->venc_video_signal, &media_config->venc_video_signal, sizeof(RTVencVideoSignal));

	if (media_config->vbr == 0)
		pvenc_config->rc_mode = VENC_COMP_RC_MODE_CBR;
	else if (media_config->vbr == 1) {
		pvenc_config->rc_mode = VENC_COMP_RC_MODE_VBR;
		pvenc_config->vbr_param.uMaxBitRate = media_config->vbr_param.uMaxBitRate;
		pvenc_config->vbr_param.nMovingTh    = media_config->vbr_param.nMovingTh;
		pvenc_config->vbr_param.nQuality      = media_config->vbr_param.nQuality;
		pvenc_config->vbr_param.nIFrmBitsCoef = media_config->vbr_param.nIFrmBitsCoef;
		pvenc_config->vbr_param.nPFrmBitsCoef = media_config->vbr_param.nPFrmBitsCoef;
	} else {
	...
	pvenc_config->enable_ve_isp_linkage = media_config->enable_ve_isp_linkage;
...
int ioctl_init(video_recoder *recoder)
{
	...
	if (recoder->config.enable_aiisp && recoder->config.channelId == CSI_SENSOR_0_VIPP_0)
	...
	if (recoder->config.enable_aiisp && recoder->config.channelId == CSI_SENSOR_1_VIPP_0)
...
int ioctl_config(video_recoder *recoder, rt_media_config_s *config)
{
	...
	cedar_get_ve_setting_info(&proc_ve_info, config->channelId);
	if (proc_ve_info.bve_proc_setting) {
		if (proc_ve_info.src_w && proc_ve_info.src_h) {
			config->width = proc_ve_info.src_w;
			config->height = proc_ve_info.src_h;
			config->dst_width = proc_ve_info.src_w;
			config->dst_height = proc_ve_info.src_h;
			RT_LOGW("get from ve proc w %d h %d", config->width, config->height);
		}
	}
	...
	vi_config.enable_aiisp = media_config->enable_aiisp;
	...
	RT_LOGD("ch%d enable_aiisp:%d", config->channelId, media_config->enable_aiisp);
	if (media_config->enable_aiisp) {
...
int ioctl_destroy(video_recoder *recoder)
{
	...
	if (recoder->config.enable_aiisp)
					     ...
					     catch_jpeg_config *jpeg_config)
{
	...
	venc_config.rc_mode = VENC_COMP_RC_MODE_CBR;
	...
	if (0 < jpeg_config->overlay_info.osd_num) {
		...
		ret = comp_set_config(recoder->venc_comp, COMP_INDEX_VENC_CONFIG_SET_OSD, (void *)&jpeg_config->overlay_info);
	}
...
static long fops_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_START: {
		...
		if (recoder->config.output_mode == OUTPUT_MODE_STREAM  || recoder->config.output_mode == OUTPUT_MODE_YUV) {
	...
	case IOCTL_GET_STREAM_DATA: {
		...
		if (video_stream == NULL) {
			...
			wait_event_timeout(recoder->stream_buf_manager.wait_bitstream,
					   recoder->stream_buf_manager.wait_bitstream_condition, HZ/5);
			return -EFAULT;
		}
	...
	case IOCTL_SET_OSD: {
		if (recoder->config.output_mode != OUTPUT_MODE_YUV) {
			...
			if (copy_from_user(&inputOsd, (void __user *)arg, sizeof(VideoInputOSD))) {
				RT_LOGE("set osd copy_from_user fail\n");
				return -EFAULT;
			}

			ret = comp_set_config(recoder->venc_comp, COMP_INDEX_VENC_CONFIG_SET_OSD, (void *)&inputOsd);
			if (ret != 0) {
				RT_LOGE("IOCTL_SET_OSD failed");
				return -EFAULT;
			}
		} else {
			RT_LOGE("IOCTL_SET_OSD is not support OUTPUT_MODE_YUV, please use IOCTL_CATCH_JPEG_START");
			return -EFAULT;
		}
		break;
	}
...
//	}
	...
	case IOCTL_SET_ISP_ATTR_CFG: {
		RTIspCtrlAttr *p_ctrlattr = vmalloc(sizeof(RTIspCtrlAttr));
		...
		ret = comp_set_config(recoder->vi_comp, COMP_INDEX_VI_CONFIG_Dynamic_SET_ISP_ARRT_CFG, (void *)p_ctrlattr);
		if (ret < 0) {
	...
	case IOCTL_GET_ISP_ATTR_CFG: {
		...
		ret = comp_get_config(recoder->vi_comp, COMP_INDEX_VI_CONFIG_Dynamic_GET_ISP_ARRT_CFG, (void *)p_ctrlattr);
		if (ret < 0) {
	...
	case IOCTL_GET_SENSOR_RESERVE_ADDR: {
		...
		if (copy_to_user((void *)arg, p_sensor_isp_config, sizeof(SENSOR_ISP_CONFIG_S))) {
			RT_LOGE("IOCTL_GET_SENSOR_RESERVE_ADDR copy_to_user fail\n");
	...
	case IOCTL_SET_CAMERA_MOVE_STATUS: {
		RT_VENC_CAMERA_MOVE_STATUS camera_move_status = (RT_VENC_CAMERA_MOVE_STATUS)arg;
		if (recoder->venc_comp) {
	...
	case IOCTL_SET_VENC_TARTGET_BITS_CLIP_PARAM: {
		...
		memset(&rt_venc_target_bits_clip_param, 0, sizeof(RT_VENC_TARGET_BITS_CLIP_PARAM));
		if (copy_from_user(&rt_venc_target_bits_clip_param, (void __user *)arg, sizeof(RT_VENC_TARGET_BITS_CLIP_PARAM))) {
		...
		if (recoder->venc_comp) {
			ret = comp_set_config(recoder->venc_comp, COMP_INDEX_VENC_CONFIG_TARGET_BITS_CLIP_PARAM, (void *)&rt_venc_target_bits_clip_param);
				if (ret != 0) {
					RT_LOGE("VENC COMP_INDEX_VENC_CONFIG_TARGET_BITS_CLIP_PARAM fail\n");
					return -EFAULT;
				}
		}
		break;
	...
	case IOCTL_SET_TDM_DROP_FRAME: {
		...
		if (copy_from_user(&drop_num, (void __user *)arg, sizeof(drop_num))) {
			RT_LOGE("IOCTL_SET_TDM_DROP_FRAME copy_from_user fail");
			return -EFAULT;
		}
		RT_LOGI("IOCTL_SET_TDM_DROP_FRAME drop_num = %d", drop_num);
		ret = comp_set_config(recoder->vi_comp, COMP_INDEX_VI_CONFIG_SET_TDM_DROP_FRAME, (void *)&drop_num);
		if (ret < 0) {
			RT_LOGE("IOCTL_SET_TDM_DROP_FRAME failed, ret = %d", ret);
			return -EFAULT;
		}
		break;
	}
	case IOCTL_SET_INPUT_BIT_WIDTH_START: {
		RT_VIN_SET_BIT_WIDTH bit_width;
		if (copy_from_user(&bit_width, (void __user *)arg, sizeof(bit_width))) {
			RT_LOGE("IOCTL_SET_INPUT_BIT_WIDTH_START copy_from_user fail");
			return -EFAULT;
		}
		RT_LOGI("IOCTL_SET_INPUT_BIT_WIDTH_START bit_width = %d", bit_width);
		ret = comp_set_config(recoder->vi_comp, COMP_INDEX_VI_CONFIG_SET_INPUT_BIT_WIDTH_START, (void *)&bit_width);
		if (ret < 0) {
			RT_LOGE("IOCTL_SET_INPUT_BIT_WIDTH_START failed, ret = %d", ret);
			return -EFAULT;
		}
		break;
	}
	case IOCTL_SET_INPUT_BIT_WIDTH_STOP: {
		RT_VIN_SET_BIT_WIDTH bit_width;
		if (copy_from_user(&bit_width, (void __user *)arg, sizeof(bit_width))) {
			RT_LOGE("IOCTL_SET_INPUT_BIT_WIDTH_STOP copy_from_user fail");
			return -EFAULT;
		}
		RT_LOGI("IOCTL_SET_INPUT_BIT_WIDTH_STOP bit_width = %d", bit_width);
		ret = comp_set_config(recoder->vi_comp, COMP_INDEX_VI_CONFIG_SET_INPUT_BIT_WIDTH_STOP, (void *)&bit_width);
		if (ret < 0) {
			RT_LOGE("IOCTL_SET_INPUT_BIT_WIDTH_STOP failed, ret = %d", ret);
			return -EFAULT;
		}
		break;
	}
