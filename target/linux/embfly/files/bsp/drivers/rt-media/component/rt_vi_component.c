...
int rt_is_lbc_format(vi_comp_ctx *vi_comp)
{
...
int rt_need_fill_data(vi_comp_ctx *vi_comp)
{
...
static int alloc_vin_buf(vi_comp_ctx *vi_comp)
{
	...
	//}
	{
		...
		for (i = 0; i < buf_num; i++) {
			...
			vi_comp->vin_buf[i].dmabuf = ion_alloc(buf_size, ION_HEAP_TYPE_DMA_MASK, 0);
			vi_comp->vin_buf[i].dmabuf_fd = dma_buf_fd(vi_comp->vin_buf[i].dmabuf, O_CLOEXEC);
			if (vi_comp->vin_buf[i].dmabuf_fd < 0) {
				RT_LOGE("cma alloc failed\n");
...
int convert_to_v4l2_color_space(RTVencVideoSignal *venc_video_signal)
{
...
int request_yuv_frame(struct vi_comp_ctx *vi_comp, rt_yuv_info *p_yuv_info)
{
	unsigned long flags;
	int ret			   = -1;
	struct vin_buffer *vin_buf = NULL;
...
int return_yuv_frame(struct vi_comp_ctx *vi_comp, unsigned char *phy_addr)
{
	int i = 0;
	int tmp_fd = -1;
	for (i = 0; i < CONFIG_YUV_BUF_NUM; i++) {
...
int post_msg_and_wait(vi_comp_ctx *vi_comp, int cmd_id, int msg_param)
{
	message_t cmd_msg;
...
error_type vi_comp_destroy(PARAM_IN comp_handle component)
{
	...
	for (i = 0; i < vi_comp->base_config.vin_buf_num; i++) {
		if (vi_comp->vin_buf[i].paddr) {
			...
			ion_heap_unmap_kernel(NULL, vi_comp->vin_buf_user[i].dmabuf->priv);
			ion_free(vi_comp->vin_buf_user[i].dmabuf->priv);
		}
	}
