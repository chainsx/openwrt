		...
		unsigned int nData1, unsigned int nData2, void *pEventData)
{
	...
	if (VencEvent_UpdateIspToVeParam == eEvent) {
		...
		RTIspCfgAttrData *p_rt_isp_cfg = venc_comp->p_rt_isp_cfg;
		...
		p_rt_isp_cfg->cfg_id = RT_ISP_CTRL_VENC2ISP_PARAM;
...
static int vencoder_create(venc_comp_ctx *venc_comp)
{
	...
	if (venc_comp->vencoder) {
		if (VENC_CODEC_H264 == type || VENC_CODEC_H265 == type) {
			if (RTVENC_PRODUCT_NUM <= venc_comp->base_config.product_mode ||
...
error_type venc_comp_component_init(PARAM_IN comp_handle component, const rt_media_config_s *pmedia_config)
{
	...
	venc_comp->p_rt_isp_cfg = vmalloc(sizeof(RTIspCfgAttrData));
	memset(venc_comp->p_rt_isp_cfg, 0, sizeof(RTIspCfgAttrData));
#if ENABLE_SAVE_NATIVE_OVERLAY_DATA
	if (global_osd_info == NULL) {
			...
			while (1) {
				...
				if (result != 0) {
					if (venc_comp->base_config.frame_rate) {
						RT_LOGI("no output buff, will wait for one frame half time %d ms fps %d.", 1000 / venc_comp->base_config.frame_rate / 2, venc_comp->base_config.frame_rate);
