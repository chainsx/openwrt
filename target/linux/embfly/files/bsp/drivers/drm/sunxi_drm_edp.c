...
static void edp_phy_enable(struct sunxi_drm_edp *drm_edp, bool en)
{
	if (en) {
		if (drm_edp->aux_phy)
			phy_power_on_force(drm_edp->aux_phy);

		if (drm_edp->combo_phy)
			phy_power_on_force(drm_edp->combo_phy);

		if (drm_edp->dp_phy) {
			phy_power_on_force(drm_edp->dp_phy);
		}
	} else {
		if (drm_edp->aux_phy)
			phy_power_off_force(drm_edp->aux_phy);

		if (drm_edp->combo_phy)
			phy_power_off_force(drm_edp->combo_phy);

		if (drm_edp->dp_phy)
			phy_power_off_force(drm_edp->dp_phy);
	}
}
...
static void edp_phy_init(struct sunxi_drm_edp *drm_edp)
{
	...
	if (drm_edp->aux_phy)
		phy_init_force(drm_edp->aux_phy);
...
{
	if (drm_edp->aux_phy)
		phy_exit_force(drm_edp->aux_phy);

	if (drm_edp->dp_phy)
		phy_exit_force(drm_edp->dp_phy);
