...
int de_gamma_set_size(struct de_gamma_handle *hdl, u32 width, u32 height)
{
	...
	reg->hori_win.bits.start = width * priv->demo_hor_start / 100;
	reg->hori_win.bits.end = width * priv->demo_hor_end / 100;
	reg->vert_win.bits.start = height * priv->demo_ver_start / 100;
	reg->vert_win.bits.end = height * priv->demo_ver_end / 100;
	gamma_set_block_dirty(priv, GAMMA_DEMO_SKIN_BLK, 1);
	return 0;
}

static int de_gamma_set_csc(struct de_gamma_handle *hdl, u32 en, int *csc_coeff)
{
	struct de_gamma_private *priv = hdl->private;
	struct gamma_reg *reg = get_gamma_reg(priv);
		    ...
		    const struct de_csc_info *out_info, const struct bcsh_info *bcsh, const struct ctm_info *ctm)
{
	...
	de_gamma_set_csc(hdl, 1, csc_coeff);
	return 0;
}
...
s32 de_gamma_set_demo_mode(struct de_gamma_handle *hdl, bool enable)
{
	struct de_gamma_private *priv = hdl->private;
	struct gamma_reg *reg = get_gamma_reg(priv);
	reg->demo_ctrl.bits.demo_en = enable;
	gamma_set_block_dirty(priv, GAMMA_DEMO_SKIN_BLK, 1);
	return 0;
}
...
struct de_gamma_handle *de_gamma_create(struct module_create_info *info)
{
	...
	hdl->cm_bit_width = dsc->cm_bit_width;
