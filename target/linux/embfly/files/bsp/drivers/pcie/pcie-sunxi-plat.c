...
static void sunxi_pcie_plat_ecc_clean_pending(struct sunxi_pcie_port *pp)
{
	struct sunxi_pcie *pcie = to_sunxi_pcie_from_pp(pp);
	u32 val;

	val = sunxi_pcie_readl(pcie, PCIE_SII_INT_RES2);
	val |= RASDP_ERR_PENDING;
	sunxi_pcie_writel(val, pcie, PCIE_SII_INT_RES2);
}
...
static irqreturn_t sunxi_pcie_plat_sii_handler(int irq, void *arg)
{
	...
	if (pci->drvdata->has_pcie_ecc) {
		u32 val;
		val = sunxi_pcie_readl(pci, PCIE_SII_INT_RES2);

		if (val & PCIE_SII_INT_RES2_ECC_MASK) {
			sunxi_info(pci->dev, "attention !! rasdp uncorrect error happened : val:0x%x \r\n", val);
			sunxi_pcie_plat_ecc_clean_pending(pp);
		}
	}
...
static void sunxi_pcie_ecc_init(struct sunxi_pcie *pci)
{
	u32 val;

	val = sunxi_pcie_readl_dbi(pci, PCIE_RASDP_ERR_PROT_CTRL_OFF);

	if (val & 0x01)
		sunxi_err(pci->dev, "ECC ferture is not working val : 0x%x\n", val);
	else {
		val = sunxi_pcie_readl(pci, PCIE_SII_INT_MASK_RES2);
		val |= APP_PARITY_ERRS2_MASK | APP_PARITY_ERRS1_MASK |	APP_PARITY_ERRS0_MASK |
				SLV_RASDP_ERR_MODE_MASK	| MATR_RASDP_ERR_MODE_MASK;
		sunxi_pcie_writel(val, pci, PCIE_SII_INT_MASK_RES2);
	}
}

static ssize_t inj_error_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct sunxi_pcie *pci = NULL;

	pci = dev->driver_data;

	return sprintf(buf, "inj_error:0x%x\n", sunxi_pcie_readl_dbi(pci, PCIE_RASDP_ERR_INJ_CTRL_OFF));

}

static ssize_t inj_error_store(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	...
	sunxi_info(dev, "injection error range:0x1~0x2\n");
	if (val >= 0x1 && val <= 0x2) {
		if (val == 1)
			sunxi_pcie_writel_dbi(pci, PCIE_RASDP_ERR_INJ_CTRL_OFF, 0x1d0111);
		else
			sunxi_pcie_writel_dbi(pci, PCIE_RASDP_ERR_INJ_CTRL_OFF, 0x1d0121);
	} else {
		sunxi_info(dev, "injection error  0x%x is fail! value:0x1~0x2\n", val);
		return count;
	}

	sunxi_info(dev, "injection error succeed: paraments :0x%x\n", val);

	return count;
}
static DEVICE_ATTR(inj_error, 0644, inj_error_show, inj_error_store);
...
static int sunxi_pcie_plat_probe(struct platform_device *pdev)
{
	...
	if (pci->drvdata->has_pcie_ecc) {
		if (sunxi_sid_get_ecc_status())
			sunxi_pcie_ecc_init(pci);

		device_create_file(&pdev->dev, &dev_attr_inj_error);
	}
...
static int sunxi_pcie_plat_remove(struct platform_device *pdev)
{
	...
	if (pci->drvdata->has_pcie_ecc)
		device_remove_file(&pdev->dev, &dev_attr_inj_error);
