	...
	for (mode = startry_mode; mode <= endtry_mode; mode++) {
		for (sample_delay = SUNXI_SPI_SAMPLE_DELAY_CHAIN_MIN;
			...
			if (block >= uboot_start)
				block = 0;
			ret = mtd->_read(mtd, (block % uboot_start) * phy_blk_size,
						len, &retlen, (u_char *)boot0_head);
...
static int spinand_download_boot_param(struct spi_device *spi, struct mtd_info *mtd)
{
	...
	struct aw_spinand_chip *chip = spinand_to_chip(spinand);
	struct aw_spinand_info *info = chip->info;
	unsigned int erase_size = info->block_size(chip);
	unsigned int phy_blk_size = info->phy_block_size(chip);
	...
	aw_spinand_uboot_blknum(spinand, NULL, &boot_param_entry);

	boot_param_entry += PHY_BLKS_FOR_SECURE_STORAGE;

#if IS_ENABLED(CONFIG_AW_MTD_SPINAND_FASTBOOT)
	boot_param_entry += 2 * ((CONFIG_BIMAGE_SIZE * 1024 * 1024) / phy_blk_size);
#endif
	instr.addr = boot_param_entry * phy_blk_size;
	instr.len = erase_size;
	ret = mtd->_erase(mtd, &instr);
	if (ret)
		goto err0;
	ret = mtd->_write(mtd, boot_param_entry * phy_blk_size,
...
static int sunxi_spi_nand_update_sample_delay_para(struct mtd_info *mtd, struct spi_device *spi)
{
	...
	boot_param_entry += PHY_BLKS_FOR_SECURE_STORAGE;
#if IS_ENABLED(CONFIG_AW_MTD_SPINAND_FASTBOOT)
	boot_param_entry += 2 * ((CONFIG_BIMAGE_SIZE * 1024 * 1024) / phy_blk_size); /* consider boot and boot_up in fastboot */
#endif
	...
	if (sunxi_spi_calibrate_get_bus_sample_mode(spi) == SUNXI_SPI_SAMP_MODE_AUTO) {
		...
		ret = mtd->_read(mtd, boot_param_entry * phy_blk_size, BOOT_PARAM_SIZE, &retlen, (u_char *)boot_param);
		if (ret && ret != ECC_LIMIT) {
						...
						void *buf, int downloadflag)
{
	...
	struct aw_spinand_chip *chip = spinand_to_chip(spinand);
	struct aw_spinand_info *info = chip->info;
	unsigned int start, end, blks_per_bootpart;
	unsigned int phy_blk_size;

	aw_spinand_uboot_blknum(spinand, NULL, &start);
	phy_blk_size = info->phy_block_size(chip);
	blks_per_bootpart = ((CONFIG_BIMAGE_SIZE * 1024 * 1024) / phy_blk_size);
	if (downloadflag) {
		end = start + blks_per_bootpart;
	} else {
		start += blks_per_bootpart;
		end = start + blks_per_bootpart;
	}
