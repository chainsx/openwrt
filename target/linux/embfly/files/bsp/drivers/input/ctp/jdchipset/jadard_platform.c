...
static int jadard_regulator_configure(struct jadard_i2c_platform_data *pdata)
{
	int retval;
	struct i2c_client *client = pjadard_ts_data->client;
	pdata->vcc_dig = regulator_get(&client->dev, "vcc_dig");
	...
	pdata->vdd_ana = regulator_get(&client->dev, "vdd_ana");
	...
	if (evdata && evdata->data &&
		...
		case 10:
			if (FB_EVENT_BLANK == event) {
				JD_I("resume  A523 : event   = %lu, TP_RESUME\n", event);
				jadard_common_resume(ts->dev);
			} else {
				JD_I("resume A523 : event = %lu, Skipped\n", event);
			}

			break;
		case 9:
			if (FB_EVENT_BLANK == event) {
				JD_I("suspend A523 : event = %lu, TP_SUSPEND\n", event);
				jadard_common_suspend(ts->dev);
			} else {
				JD_I("suspend A523 : event = %lu, Skipped\n", event);
			}
			break;
		}
	}
