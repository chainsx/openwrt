...
/*** add support regulator power && io 3.3 to 1.8 en by lxm 2023.10.11 ***/ 
#ifdef CFG_CTS_HAS_REGULATOR
void cts_plat_regulator_power_on(struct cts_platform_data *pdata ,bool status)
{
	cts_info("%s-%s,regu_count=%d\n", __func__,status ? "POWER ON" : "POWER OFF",pdata->regulator_count);

	if (status) {
		if (pdata->vcc) {
			if (regulator_enable(pdata->vcc) < 0)
				cts_err("regulator_enable VCC fail\n");
			else
				pdata->regulator_count ++;
		}
	} else {
		if (pdata->vcc && (pdata->regulator_count > 0)) {
			if (regulator_disable(pdata->vcc) < 0)
				cts_err("regulator_enable VCC fail\n");
			else
				pdata->regulator_count --;
		}
...
static void cts_plat_regulator_power_init(struct cts_platform_data *pdata)
{
	const char *vcc_name = "vcc";

	pdata->vcc = regulator_get(&(pdata->i2c_client->dev), vcc_name);
	if (IS_ERR(pdata->vcc)) {
		cts_err("regulator_get VCC fail\n");
		pdata->vcc = NULL;
	}
	if (pdata->vcc)
	{
		regulator_set_voltage(pdata->vcc, VCC_VOLTAGE, VCC_VOLTAGE);
	}
	pdata->regulator_count = 0;
	//cts_plat_regulator_power_on(pdata,true);
}
#endif

#ifdef CFG_CTS_HAS_IO_EN_PIN
void cts_plat_io_en(struct cts_platform_data *pdata ,bool status)
{
	cts_info("%s-%s\n", __func__,status ? "POWER ON" : "POWER OFF");

	if (status) {
		if (gpio_is_valid(pdata->io_en_gpio)) {
			cts_info("io_en_gpio :1s %d\n",pdata->io_en_gpio);
			gpio_direction_output(pdata->io_en_gpio, 1);
			gpio_set_value(pdata->io_en_gpio, 1);
			cts_info("io_en_gpio :1d %d\n",pdata->io_en_gpio);
		}
	} else {
		cts_info("io_en_gpio :0 %d\n",pdata->io_en_gpio);
		if (gpio_is_valid(pdata->io_en_gpio)) {
			gpio_direction_output(pdata->io_en_gpio, 0);
			gpio_set_value(pdata->io_en_gpio, 0);
		}
	}
	mdelay(10);
}
...
void cts_plat_power_on(struct cts_platform_data *pdata ,bool status)
{
	#ifdef CFG_CTS_HAS_REGULATOR
	cts_plat_regulator_power_on(pdata,status);
	#endif
	#ifdef CFG_CTS_HAS_IO_EN_PIN
	cts_plat_io_en(pdata,status);
	#endif
}
...
#ifdef CFG_CTS_HAS_IO_EN_PIN
	pdata->io_en_gpio = of_get_named_gpio(dev_node, CFG_CTS_OF_IO_EN_GPIO_NAME, 0);
    if (!gpio_is_valid(pdata->io_en_gpio)) {
        cts_err("Parse IO_EN GPIO from dt failed %d", pdata->io_en_gpio);
        pdata->io_en_gpio = -1;
    }
    cts_info("  %-12s: %d", "IO_EN gpio", pdata->io_en_gpio);
#endif
...
#else
	pdata->res_x = CTS_DEFAULT_MAX_WIDTH;
	pdata->res_y = CTS_DEFAULT_MAX_HEIGHT;
	cts_info("  %-12s: %d", "X resolution", pdata->res_x);
	cts_info("  %-12s: %d", "Y resolution", pdata->res_y);
#endif
...
#ifdef CFG_CTS_HAS_REGULATOR
	cts_plat_regulator_power_init(pdata);
#endif
...
#ifdef CFG_CTS_HAS_IO_EN_PIN
	if (gpio_is_valid(pdata->io_en_gpio)) {
		ret = gpio_request_one(pdata->io_en_gpio, GPIOF_OUT_INIT_HIGH,
				CFG_CTS_DEVICE_NAME "-io_en");
		if (ret) {
			cts_err("Request io_en gpio (%d) failed %d", pdata->io_en_gpio, ret);
			goto err_free_io_en;
		}
	}
#endif /* CFG_CTS_HAS_IO_EN_PIN */
...
/*** add support regulator power && io 3.3 to 1.8 en by lxm 2023.10.11 ***/
	cts_plat_power_on(pdata,false);

#ifdef CFG_CTS_HAS_REGULATOR
	if (!IS_ERR_OR_NULL(pdata->vcc)) {
        ...
        struct cts_device_touch_msg *msgs, int num)
{
    ...
    for (i = 0; i < num; i++) {
		...
		/*** add support x y swap func by lxm 2023.09.01 ***/
		#ifdef SWAP_XY
			swap(x, y);
		#endif
