...
#define SUNXI_CCU_VERSION		"1.0.0"
...
static struct ccu_nm pll_audio0_4x_clk = {
	...
	.sdm		= _SUNXI_CCU_SDM(pll_audio0_4x_sdm_table, BIT(23),    /* The sdm enable bit has been replaced with another register, so an invalid bit(23) is used here to replace it */
			0x0268, BIT(31)),
	.min_rate	= 90316800,
	.max_rate	= 90316800,
	.common		= {
...
static SUNXI_CCU_GATE(cpus_hclk_gate_clk, "cpus-hclk-gate",
		"dcxo",
		0x05C0, BIT(28), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(store_ahb_gate_clk, "store-ahb-gate",
		"dcxo",
		0x05C0, BIT(24), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(msilite0_ahb_gate_clk, "msilite0-ahb-gate",
		"dcxo",
		0x05C0, BIT(16), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(usb_sys_ahb_gate_clk, "usb-sys-ahb-gate",
		"dcxo",
		0x05C0, BIT(9), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(serdes_ahb_gate_clk, "serdes-ahb-gate",
		"dcxo",
		0x05C0, BIT(8), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(gpu0_ahb_gate_clk, "gpu0-ahb-gate",
		"dcxo",
		0x05C0, BIT(7), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(npu_ahb_gate_clk, "npu-ahb-gate",
		"dcxo",
		0x05C0, BIT(6), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(de_ahb_gate_clk, "de-ahb-gate",
		"dcxo",
		0x05C0, BIT(5), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(vid_out1_ahb_gate_clk, "vid-out1-ahb-gate",
		"dcxo",
		0x05C0, BIT(4), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(vid_out0_ahb_gate_clk, "vid-out0-ahb-gate",
		"dcxo",
		0x05C0, BIT(3), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(vid_in_ahb_gate_clk, "vid-in-ahb-gate",
		"dcxo",
		0x05C0, BIT(2), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(ve_enc_ahb_gate_clk, "ve-enc-ahb-gate",
		"dcxo",
		0x05C0, BIT(1), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(ve_ahb_gate_clk, "ve-ahb-gate",
		"dcxo",
		0x05C0, BIT(0), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(msilite2_mbus_gate_clk, "msilite2-mbus-gate",
		"dcxo",
		0x05E0, BIT(31), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(store_mbus_gate_clk, "store-mbus-gate",
		"dcxo",
		0x05E0, BIT(30), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(msilite0_mbus_gate_clk, "msilite0-mbus-gate",
		"dcxo",
		0x05E0, BIT(29), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(serdes_mbus_gate_clk, "serdes-mbus-gate",
		"dcxo",
		0x05E0, BIT(28), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(vid_in_mbus_gate_clk, "vid-in-mbus-gate",
		"dcxo",
		0x05E0, BIT(24), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(npu_mbus_gate_clk, "npu-mbus-gate",
		"dcxo",
		0x05E0, BIT(18), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(gpu0_mbus_gate_clk, "gpu0-mbus-gate",
		"dcxo",
		0x05E0, BIT(16), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(ve_dec_mbus_gate_clk, "ve-dec-mbus-gate",
		"dcxo",
		0x05E0, BIT(14), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(ve_mbus_gate_clk, "ve-mbus-gate",
		"dcxo",
		0x05E0, BIT(12), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(desys_mbus_gate_clk, "desys-mbus-gate",
		"dcxo",
		0x05E0, BIT(11), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(iommu1_mbus_gate_clk, "iommu1-mbus-gate",
		"dcxo",
		0x05E0, BIT(1), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(iommu0_mbus_gate_clk, "iommu0-mbus-gate",
		"dcxo",
		0x05E0, BIT(0), CLK_IS_CRITICAL);

static SUNXI_CCU_GATE(ve_dec_mbus_clk, "ve-dec-mclk",
		"dcxo",
		0x05E4, BIT(18), 0);

static SUNXI_CCU_GATE(gmac1_mbus_clk, "gmac1-mclk",
		"dcxo",
		0x05E4, BIT(12), 0);

static SUNXI_CCU_GATE(gmac0_mbus_clk, "gmac0-mclk",
		"dcxo",
		0x05E4, BIT(11), 0);

static SUNXI_CCU_GATE(isp_mbus_clk, "isp-mclk",
		"dcxo",
		0x05E4, BIT(9), 0);

static SUNXI_CCU_GATE(csi_mbus_clk, "csi-mclk",
		"dcxo",
		0x05E4, BIT(8), 0);

static SUNXI_CCU_GATE(nand_mbus_clk, "nand-mclk",
		"dcxo",
		0x05E4, BIT(5), 0);

static SUNXI_CCU_GATE(dma1_mbus_clk, "dma1-mclk",
		"dcxo",
		0x05E4, BIT(3), 0);

static SUNXI_CCU_GATE(ce_mbus_clk, "ce-mclk",
		"dcxo",
		0x05E4, BIT(2), CLK_IGNORE_UNUSED);

static SUNXI_CCU_GATE(ve_mbus_clk, "ve-mclk",
		"dcxo",
		0x05E4, BIT(1), 0);

static SUNXI_CCU_GATE(dma0_mbus_clk, "dma0-mclk",
		"dcxo",
		0x05E4, BIT(0), 0);
