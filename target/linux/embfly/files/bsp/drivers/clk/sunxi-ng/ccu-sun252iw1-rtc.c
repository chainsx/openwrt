...
static struct ccu_common *sun252iw1_rtc_ccu_clks[] = {
	&iosc_clk.common,
...
static struct clk_hw_onecell_data sun252iw1_rtc_ccu_hw_clks = {
	.hws	= {
...
static const struct sunxi_ccu_desc sun252iw1_rtc_ccu_desc = {
	.ccu_clks	= sun252iw1_rtc_ccu_clks,
	.num_ccu_clks	= ARRAY_SIZE(sun252iw1_rtc_ccu_clks),

	.hw_clks	= &sun252iw1_rtc_ccu_hw_clks,
};
...
static int sun252iw1_rtc_ccu_probe(struct platform_device *pdev)
{
	struct resource *res;
	...
	return sunxi_ccu_probe(pdev->dev.of_node, reg, &sun252iw1_rtc_ccu_desc);
}

static const struct of_device_id sun252iw1_rtc_ccu_ids[] = {
	{ .compatible = "allwinner,sun252iw1-rtc-ccu" },
	{ }
};

static struct platform_driver sun252iw1_rtc_ccu_driver = {
	.probe	= sun252iw1_rtc_ccu_probe,
	.driver	= {
		.name	= "sun252iw1-rtc-ccu",
		.of_match_table	= sun252iw1_rtc_ccu_ids,
	},
};

static int __init sun252iw1_rtc_ccu_init(void)
{
	int err;

	err = platform_driver_register(&sun252iw1_rtc_ccu_driver);
	if (err)
		pr_err("Fail to register sunxi_rtc_ccu as platform device\n");

	return err;
}
core_initcall(sun252iw1_rtc_ccu_init);

static void __exit sun252iw1_rtc_ccu_exit(void)
{
	platform_driver_unregister(&sun252iw1_rtc_ccu_driver);
}
module_exit(sun252iw1_rtc_ccu_exit);
