...
static struct ccu_common *sun55iw6_rtc_ccu_clks[] = {
...
static int sun55iw6_rtc_ccu_probe(struct platform_device *pdev)
{
	struct resource *res;
	...
	return sunxi_ccu_probe(pdev->dev.of_node, reg, &sun55iw6_rtc_ccu_desc);
}

static const struct of_device_id sun55iw6_rtc_ccu_ids[] = {
	{ .compatible = "allwinner,sun55iw6-rtc-ccu" },
	{ }
};

static struct platform_driver sun55iw6_rtc_ccu_driver = {
	.probe	= sun55iw6_rtc_ccu_probe,
	.driver	= {
		.name	= "sun55iw6-rtc-ccu",
		.of_match_table	= sun55iw6_rtc_ccu_ids,
	},
};

static int __init sun55iw6_rtc_ccu_init(void)
{
	int err;

	err = platform_driver_register(&sun55iw6_rtc_ccu_driver);
	if (err)
		pr_err("Fail to register sunxi_rtc_ccu as platform device\n");

	return err;
}
core_initcall(sun55iw6_rtc_ccu_init);

static void __exit sun55iw6_rtc_ccu_exit(void)
{
	platform_driver_unregister(&sun55iw6_rtc_ccu_driver);
}
module_exit(sun55iw6_rtc_ccu_exit);
