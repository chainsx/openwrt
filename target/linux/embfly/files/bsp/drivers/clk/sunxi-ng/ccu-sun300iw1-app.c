...
static const char * const spi2_clk_parents[] = { "dcxo", "pll-peri-cko-cko-307m", "pll-peir-cko-236m", "", "pll-peri-cko-cko-48m", "pll-csi-2x" };
...
static const char * const smhc1_parents[] = { "dcxo", "pll-peri-cko-192m", "pll-peri-cko-219m", "", "pll-ddr", "", "pll-vedio0-2x" };
...
static const char * const pll_audio_1x_parents[] = { "pll-peri-cko-cko-614", "pll-cpu", "pll-video-2x" };
...
static int sun300iw1_app_ccu_probe(struct platform_device *pdev)
{
	...
	ret = sunxi_ccu_probe(pdev->dev.of_node, reg, &sun300iw1_app_ccu_desc);
	if (ret)
		return ret;
...
static int __init sun300iw1_app_ccu_init(void)
{
	...
	if (err)
		pr_err("register ccu sun300iw1_app failed\n");
