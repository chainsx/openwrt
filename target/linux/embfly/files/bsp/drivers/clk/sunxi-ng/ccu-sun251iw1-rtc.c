...
static struct ccu_common *sun251iw1_rtc_ccu_clks[] = {
	&iosc_clk.common,
...
static struct clk_hw_onecell_data sun251iw1_rtc_ccu_hw_clks = {
	.hws	= {
...
static const struct sunxi_ccu_desc sun251iw1_rtc_ccu_desc = {
	.ccu_clks	= sun251iw1_rtc_ccu_clks,
	.num_ccu_clks	= ARRAY_SIZE(sun251iw1_rtc_ccu_clks),

	.hw_clks	= &sun251iw1_rtc_ccu_hw_clks,
};
...
static int sun251iw1_rtc_ccu_probe(struct platform_device *pdev)
{
	struct resource *res;
	...
	return sunxi_ccu_probe(pdev->dev.of_node, reg, &sun251iw1_rtc_ccu_desc);
}

static const struct of_device_id sun251iw1_rtc_ccu_ids[] = {
	{ .compatible = "allwinner,sun251iw1-rtc-ccu" },
	{ }
};

static struct platform_driver sun251iw1_rtc_ccu_driver = {
	.probe	= sun251iw1_rtc_ccu_probe,
	.driver	= {
		.name	= "sun251iw1-rtc-ccu",
		.of_match_table	= sun251iw1_rtc_ccu_ids,
	},
};

static int __init sun251iw1_rtc_ccu_init(void)
{
	int err;

	err = platform_driver_register(&sun251iw1_rtc_ccu_driver);
	if (err)
		pr_err("Fail to register sunxi_rtc_ccu as platform device\n");

	return err;
}
core_initcall(sun251iw1_rtc_ccu_init);

static void __exit sun251iw1_rtc_ccu_exit(void)
{
	platform_driver_unregister(&sun251iw1_rtc_ccu_driver);
}
module_exit(sun251iw1_rtc_ccu_exit);
