...
#include <../crashdump/sunxi-crashnote.h>
...
static void sunxi_store_pregs_trigger(void *p, struct pt_regs *regs)
{
...
static int __init sunxi_vendor_hooks_init(void)
{
	...
	ret = register_trace_android_vh_ipi_stop(sunxi_store_pregs_trigger, NULL);
	if (ret)
		pr_err("%s: register sunxi_store_pregs_trigger failed!\n", __func__);
