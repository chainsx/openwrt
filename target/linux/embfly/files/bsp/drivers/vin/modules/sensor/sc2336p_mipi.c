...
/*
 * A V4L2 driver for sc2336p Raw cameras.
 *
...
MODULE_AUTHOR("lwj");
MODULE_DESCRIPTION("A low-level driver for sc2336p sensors");
MODULE_LICENSE("GPL");
...
/*
 * The sc2336p i2c address
 */
#define I2C_ADDR 0x30

#define SENSOR_NUM 0x2
#define SENSOR_NAME "sc2336p_mipi"
#define SENSOR_NAME_2 "sc2336p_mipi_2"
...
static struct regval_list sensor_1080p_20fps_regs[] = {
#if VIN_TRUE
	{0x0103, 0x01},
	{0x0100, 0x00},
	{0x36e9, 0x80},
...
static int sc2336p_sensor_vts;
static int sc2336p_fps_change_flag;
...
//	data_type read_high = 0, read_low = 0;
	...
	if (!sc2336p_fps_change_flag) {
		shutter = exp_val >> 4;
		if (shutter > sc2336p_sensor_vts - 8) {
			frame_length = shutter + 8;
		} else {
			frame_length = sc2336p_sensor_vts;
		}
...
//	data_type read_high = 0, read_low = 0;
	int sc2336p_sensor_target_vts = 0;

	sc2336p_fps_change_flag = 1;
	sc2336p_sensor_target_vts = wsize->pclk/fps->fps/wsize->hts;

	if (sc2336p_sensor_target_vts <= wsize->vts) {// the max fps = 20fps
		sc2336p_sensor_target_vts = wsize->vts;
	} else if (sc2336p_sensor_target_vts >= (wsize->pclk/wsize->hts)) {// the min fps = 1fps
		sc2336p_sensor_target_vts = (wsize->pclk/wsize->hts) - 8;
	}

	sc2336p_sensor_vts = sc2336p_sensor_target_vts;
	sensor_dbg("target_fps = %d, sc2336p_sensor_target_vts = %d, 0x320e = 0x%x, 0x320f = 0x%x\n", fps->fps,
		sc2336p_sensor_target_vts, sc2336p_sensor_target_vts >> 8, sc2336p_sensor_target_vts & 0xff);
	sensor_write(sd, 0x320f, (sc2336p_sensor_target_vts & 0xff));
	sensor_write(sd, 0x320e, (sc2336p_sensor_target_vts >> 8));
//	sensor_read(sd, 0x320e, &read_high);
//	sensor_read(sd, 0x320f, &read_low);
//	sensor_print("[get write_times] 0x41 = 0x%x, 0x42 = 0x%x\n", read_high, read_low);
	sc2336p_fps_change_flag = 0;
...
static int sensor_detect(struct v4l2_subdev *sd)
{
	...
	SENSOR_ID |= (rdval);
	sensor_print("sc2336p V4L2_IDENT_SENSOR = 0x%x\n", SENSOR_ID);
...
static struct sensor_win_size sensor_win_sizes[] = {
...

	{
		...
		.gain_min = 1 << 4,
		.gain_max = 128 << 4,
		.regs = sensor_1080p_20fps_regs,
		.regs_size = ARRAY_SIZE(sensor_1080p_20fps_regs),
		.set_size = NULL,
	},
};
