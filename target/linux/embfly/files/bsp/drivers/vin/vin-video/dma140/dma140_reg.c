...
int dma_virtual_find_ch[8] = {
	0, 1, 2, 3, 0, 1, 2, 3,
};
int dma_virtual_find_logic[8] = {
	0, 0, 0, 0, 4, 4, 4, 4,
};
#else
...
void csic_dma_ve_online_sensor_chn_en(unsigned int sel, unsigned int en)
{
	vin_reg_clr_set(csic_dma_base[sel] + CSIC_DMA_VE_ID_EN_REG_OFF,
			SENSOR_ID0_EN_MASK << dma_virtual_find_ch[sel], en << (SENSOR_ID0_EN + dma_virtual_find_ch[sel]));
}
...
{
	vin_reg_clr_set(csic_dma_base[sel] + CSIC_DMA_VE_ID_EN_REG_OFF,
			ID_EN_BK_DOOR_MASK, mode << ID_EN_BK_DOOR);
}
...
int __csic_dma_get_frame_cnt(unsigned int sel, unsigned int ch)
{
